...
async def _family_aggregate(
    ...
    *,
    ...
    for member_id in family.member_thread_ids:
        agg = await run_store.aggregate_tokens_by_thread(member_id, include_active=include_active, user_id=user_id)
...
async def thread_token_usage(
    ...
    **A conversation is not always one thread.** Editing a message branches it
    ...
    if family.is_single_thread:
        agg = await run_store.aggregate_tokens_by_thread(thread_id, include_active=include_active, user_id=scope_user_id)
        hidden_run_ids = await _superseded_run_ids(request, thread_id)
    else:
        agg, hidden_run_ids = await _family_aggregate(request, run_store, family, thread_id, include_active=include_active, user_id=scope_user_id)
