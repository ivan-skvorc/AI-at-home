...
# This fork's value is 250 (not LangGraph's 25, nor upstream's 100): agentic
# sessions that clone a repo and iterate — many bash / read_file / write_file /
# str_replace turns, each a super-step — routinely exceed a lower cap and abort
# with GraphRecursionError mid-task. 250 matches the autonomous github-channel
# budget; the 1000 ceiling still bounds cost. Keep it in lockstep with
# ``AppConfig.recursion_limit``'s default so a config-load failure does not
# quietly shorten runs.
_DEFAULT_RECURSION_LIMIT = 250
