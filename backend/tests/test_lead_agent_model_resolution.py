...
def test_make_lead_agent_reads_runtime_options_from_context(monkeypatch):
    ...
    get_available_tools.assert_called_once_with(model_name="context-model", groups=None, subagent_enabled=True, mcp_plugins=None, include_conversation_reader=False, app_config=app_config, chat_model=result["model"], internet_enabled=True)
    assert result["model"] is not None
