...
def test_rendered_allowed_tools_agree_with_skill_tool_policy():
    ...
    restricted = _make_skill("locked-down", allowed_tools=())
    # The rendered line describes what *activating* the skill does, which is the
    # dynamic policy (SkillToolPolicyMiddleware). This fork's static default
    # exempts an empty declaration so a merely enabled skill strips nothing.
