...
def _get_alembic_config_for_head() -> AlembicConfig:
    """Read the migration tree without needing a database URL."""
...
async def test_occurrence_revision_is_reachable_from_the_single_head():
    ...
    `0023_merge_pricing_scheduler` joins this branch to the fork's pricing
    branch (FORK.md §17), so the head is that merge point rather than this
    revision. The property upstream is protecting survives the rename: there is
    exactly one head — `_get_head_revision` raises on two — and this revision
    is on the path to it, so `upgrade head` still runs it. The merge points
    themselves are pinned by `tests/test_migration_merge_points.py`.
    """
