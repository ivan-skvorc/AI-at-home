...
# The healthy-database test below drives the ORM, which selects every mapped
# column — including this fork's `pricing_snapshot` (FORK.md §17). That column
# lives on the fork's own migration branch, so `PREVIOUS`, which sits on
# upstream's branch beside it, builds a `runs` table without it. The fork's
# merge point is the equivalent baseline here: it carries both branches, and
# everything `_assert_schema` looks at is owned by `0023_run_change_seq`, which
# it already includes.
HEALTHY_BASELINE = "0024_merge_preferences"
...
async def test_repair_preserves_clock_and_downgrade_keeps_ancestor_schema(tmp_path):
    ...
    cfg = _get_alembic_config(engine)
    try:
        await asyncio.to_thread(command.upgrade, cfg, HEALTHY_BASELINE)
        repo = RunRepository(async_sessionmaker(engine, expire_on_commit=False))
        ...
        await asyncio.to_thread(command.downgrade, cfg, HEALTHY_BASELINE)
        await _assert_schema(engine)
