...
# `. lib.sh` / `source lib.sh` reads the file into the *current* shell, which
# needs read permission and never the executable bit — so it is not the failure
# this test is about. Naming an interpreter there would be an outright bug:
# `bash lib.sh` runs it in a subshell, and the functions it defines would be
# gone by the next line. The fork sources scripts/tailscale_lib.sh this way
# from deploy.sh and docker.sh.
_SOURCE_COMMAND = re.compile(r"(?:^|\s)(?:\.|source)\s+$")
        ...
        for number, raw in enumerate(script.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            for match in _SCRIPT_REFERENCE.finditer(line):
                ...
                if _MESSAGE_COMMAND.search(before):  # a hint printed to the user
                    continue
                if _SOURCE_COMMAND.search(before):  # sourced, not executed
                    continue
