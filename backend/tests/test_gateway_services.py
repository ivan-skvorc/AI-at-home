...
def test_build_run_config_logs_and_uses_fallback_when_app_config_unavailable(monkeypatch, caplog):
    ...
    # Interpolated, not literal: this fork's default is 250, and a hardcoded
    # number here would make the assertion about upstream's value rather than
    # about the log naming the fallback it actually used.
    assert any(f"failed to load app config; falling back to recursion_limit={services._DEFAULT_RECURSION_LIMIT}" in record.message for record in caplog.records)
