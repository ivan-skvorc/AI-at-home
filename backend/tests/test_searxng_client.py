...
class TestUnresponsiveEngines:
    ...
    async def test_engines_blocked_on_a_later_page_keep_the_earlier_pages(self):
        # A max_results above one page walks pageno, and the block often lands
        # mid-walk. Raising there would discard the rows page 1 already
        # delivered and report a partial success as a total outage.
        page1 = [{"title": f"r{i}", "url": f"https://example.com/{i}", "content": "c"} for i in range(3)]
        payloads = {
            1: {"results": page1},
            2: {"results": [], "unresponsive_engines": [["brave", "too many requests"]]},
        }
        with patch("deerflow.community.searxng.searxng_client.httpx.AsyncClient") as mock_cls:
            mock_ctx = MagicMock()
            mock_cls.return_value.__aenter__.return_value = mock_ctx
            ...
            def _get(url, params=None, headers=None):
                mock_resp = MagicMock()
                mock_resp.status_code = 200
                mock_resp.json.return_value = payloads[params["pageno"]]
                mock_resp.raise_for_status.return_value = None
                return mock_resp

            mock_ctx.get = AsyncMock(side_effect=_get)
            client = SearxngClient(base_url="http://searxng:8080")
            results = await client.search("blocked mid-walk", max_results=10)

        assert [r["url"] for r in results] == [r["url"] for r in page1]
        assert mock_ctx.get.call_count == 2
