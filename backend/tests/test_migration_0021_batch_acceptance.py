...
async def test_forward_revision_cannot_skip_required_batch_columns(tmp_path, monkeypatch, race):
    ...
    cfg = bootstrap._get_alembic_config(engine)
    try:
        # Keep the project schema present so the batch-column guard is what
        # rejects this database, on both direct and concurrent-startup paths.
        # `0020_threads_meta_project_id` is on upstream's branch only, so the
        # fork's `runs.pricing_snapshot` (0019_runs_pricing_snapshot, the other
        # parent of the merge head) is legitimately absent here too and leads
        # the guard's list — hence the `.*` below rather than a looser match on
        # the batch columns themselves.
        ...
        with pytest.raises(RuntimeError, match=r"missing required local schema: .*subagent_batch_items\.acceptance_criteria, subagent_batch_items\.acceptance_verdict"):
            await bootstrap.bootstrap_schema(engine, backend="sqlite")
    finally:
        await engine.dispose()
