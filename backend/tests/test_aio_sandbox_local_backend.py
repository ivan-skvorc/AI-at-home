...
def test_start_container_expose_ports_follow_the_resolved_bind_for_dood(monkeypatch):
    """A debug port publishes wherever the sandbox API publishes.

    This used to assert a bare ``0.0.0.0``: a non-loopback sandbox host
    (Docker-outside-of-Docker via ``host.docker.internal``) bound every
    interface. Upstream hardened that — the sandbox exec API is
    unauthenticated, so it now binds the address the sandbox host actually
    resolves to — and the fork follows, because publishing a debug port on
    every interface is the exposure its own deployment check (FORK.md §12)
    exists to catch. What the fork still needs is that ``expose_ports``
    tracks the *same* bind as the API port, so a program under test stays
    reachable exactly where the sandbox is; the opt-in below restores the
    old broad bind for anyone who genuinely needs it.
    """
    backend = LocalContainerBackend(
        image="sandbox:latest",
        base_port=8080,
    ...
    port_args = _pairs(_capture_start_container_command(monkeypatch, backend), "-p")

    # First -p is the sandbox API port; its host part is the resolved bind.
    api_bind = port_args[0].rsplit(":", 2)[0]
    assert api_bind != "0.0.0.0"
    assert f"{api_bind}:8000:8000" in port_args


def test_start_container_expose_ports_honor_the_broad_bind_opt_in(monkeypatch):
    """DEER_FLOW_SANDBOX_BIND_HOST=0.0.0.0 restores the pre-hardening bind."""
    backend = LocalContainerBackend(
        image="sandbox:latest",
        base_port=8080,
