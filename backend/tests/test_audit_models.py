...
class TestStaleFixture:
    ...
    def test_the_fixture_drifts_from_the_config_only_where_it_means_to(self):
        """Every entry the config routes is in the fixture, bar the one retired on purpose.

        The fixture is the audit's only offline proof that it still detects
        drift, and it is regenerated by hand from `config.example.yaml`. A
        routed model the fixture simply *forgot* is indistinguishable in the
        output from the one it deletes deliberately: both read as `retired`.
        That is silent — the assertion above passes on any fixture that yields
        at least one finding, so the two halves of the paired-lab rule
        (`anthropic/claude-opus-5`, `openai/gpt-6-astra`) went missing for a
        roster roll-forward without any test going red, and every extra
        `retired` line is noise a reader has to re-derive as harmless.

        So pin the *shape* the fixture's own `_comment` promises: exactly one
        slug absent, and every other routed slug present.
        """
        config = (REPO_ROOT / "config.example.yaml").read_text(encoding="utf-8")
        routed = {entry.slug for entry in audit.parse_marker_blocks(config) if entry.provider == "openrouter"}

        fixture = json.loads((REPO_ROOT / "scripts" / "fixtures" / "model_audit_stale_catalog.json").read_text(encoding="utf-8"))
    ...
    def test_the_fixture_reports_one_finding_per_deliberate_drift(self, capsys):
        """Four drifts in, four findings out — no more.

        `test_..._produces_a_readable_issue` only asks for *some* finding, so a
        fixture that has fallen behind the roster still reads green while
        reporting models nothing retired. Counting the findings is what makes
        the extra ones fail.
        """
        fixture = REPO_ROOT / "scripts" / "fixtures" / "model_audit_stale_catalog.json"
        audit.main(["--catalog", str(fixture), "--format", "json"])
        ...
        assert len(findings) == 4, f"expected the four deliberate drifts, got: {[f['kind'] for f in findings]}"
