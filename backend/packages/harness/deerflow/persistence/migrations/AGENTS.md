...
**Rolling forward compatibility**: the local chain head is
`0022_merge_pricing_projects`, and the chain **branches** at
`0018_oauth_identity_pg_partial`. Upstream's side runs `0019_projects` →
`0020_threads_meta_project_id` → `0021_batch_acceptance`; the fork's side is the
single revision `0019_runs_pricing_snapshot` (FORK.md §17). Both are parents of
`0022_merge_pricing_projects`, which is a no-op merge point that exists only to
give Alembic one head — `upgrade head` refuses to run against two, and the
Gateway's bootstrap fails outright before serving a request. Do **not** resolve a
future collision of this shape by re-parenting either branch: a database already
stamped at one branch's tip would then read as being at head with the other
branch's tables never created, which is a wrong schema that fails silently until
a query hits it. Add another merge revision instead.
