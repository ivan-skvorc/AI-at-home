...
def bash_tool(runtime: Runtime, command: str, description: str = "") -> str:
    ...
    try:
        ...
        try:
            ...
            max_chars = 20000
        # No per-call timeout on the remote path: the provider is constructed
        # with `bash_command_timeout` as its `default_command_timeout`, so the
        # configured budget already reaches the sandbox. Passing it again here
        # would make one setting arrive by two routes, which is how they drift.
        return _truncate_bash_output(
            mask_secret_values(
...
def read_file_tool(
    ...
    try:
        ...
        return _truncate_read_file_output(content, _context_clamped(max_chars, READ_FILE_SHARE), line_offset=effective_start - 1, joined_lines=use_line_range, ends_at_eof=end_line is None)
    except SandboxError as e:
        return f"Error: {e}"
    except FileNotFoundError:
