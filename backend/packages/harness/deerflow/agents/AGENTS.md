...
**System prompt** (`lead_agent/prompt.py`): a user-authored override at
`{base_dir}/SYSTEM_PROMPT.md` replaces the built-in template, re-read on every
agent build. Fork feature (FORK.md §19); the placeholder, validation and test
rules live beside the code in [`lead_agent/AGENTS.md`](lead_agent/AGENTS.md).
...
- `democracy_participants` - Fork: Democracy panel roster; invariants in [`lead_agent/AGENTS.md`](lead_agent/AGENTS.md)
