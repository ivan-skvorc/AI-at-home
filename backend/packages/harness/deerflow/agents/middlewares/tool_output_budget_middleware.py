...
class ToolOutputBudgetMiddleware(AgentMiddleware[AgentState]):
    ...
    def release_policy_parameters(self) -> dict[str, object]:
        ...
        return {
            ...
            "skill_read_tool_names": sorted(self._skill_read_tool_names),
    ...
    def wrap_tool_call(
        ...
        if config.enabled and _needs_budget(result, config):
            outputs_path = _resolve_outputs_path(request)
            sandbox = _resolve_sandbox(request)
            result = _patch_result(result, config, outputs_path, sandbox)
    ...
    async def awrap_tool_call(
        ...
        if config.enabled and _needs_budget(result, config):
            ...
            sandbox = _resolve_sandbox(request)
            result = await asyncio.to_thread(_patch_result, result, config, outputs_path, sandbox)
        return _record_visible_skill_usage(result, request, skill_read_tool_names=self._skill_read_tool_names, skills_root=self._skills_root)
    ...
    def _budget_model_request(self, request: ModelRequest) -> ModelRequest:
        ...
        # Resolve per request, not from ``self._config``: the thresholds are
        # scaled down to the serving model's context window, and the serving
        # model is only known once the request arrives.
        ...
        if patched is not None:
            messages = patched
        if config.elide_superseded_writes:
            elided = elide_superseded_write_payloads(
                messages,
                min_chars=config.superseded_write_min_chars,
                keep_recent=config.keep_recent_writes,
            )
