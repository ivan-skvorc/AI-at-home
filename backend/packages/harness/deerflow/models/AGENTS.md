...
- **`price:` / `discount:` (§17)** — the operator-facing cost surface on a model entry: `price: {currency, input, output, cache_hit}` and `discount: {input, output, cache_hit, until}`, per one million tokens. `pricing.py::_raw_from_price_fields` normalizes them into the same internal shape the legacy `pricing:` block uses, so precedence is `price:` > `pricing:` > `derive_pricing_from_display_name` with **one** cost path rather than three. The discount is strictly additive — spend bills at `price`, the discount is shown beside it.
...
- Tests: `tests/test_model_price_fields.py`, `tests/test_pricing.py`, `tests/test_model_fallback.py`.
