    ...
    async def search(
        ...
        for pageno in range(1, self._MAX_PAGES + 1):
            try:
                ...
                # Engines blocked on a later page must not discard the rows an
                # earlier page already delivered; only a first page with
                # nothing to show is the failure the agent has to back off from.
                if collected:
                    break
                raise
            if not rows:
