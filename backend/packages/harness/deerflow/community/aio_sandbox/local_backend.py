...
class LocalContainerBackend(SandboxBackend):
    ...
    def _start_container(
        ...
        *,
        ...
        # Skipped whenever the sandbox port itself is not published: restricted
        # network mode puts the container on an internal network reachable only
        # through the relay, so a host publish here would be both useless and a
        # hole in the isolation the mode exists to provide.
        for extra_port in self._expose_ports if publish_port else []:
            if self._runtime == "docker":
