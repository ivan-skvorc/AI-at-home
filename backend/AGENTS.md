...
- **Tool policy (`skills/tool_policy.py`)**: `allowed-tools` frontmatter scopes a skill's own *business* tools; it is enforced globally at agent-build time via `filter_tools_by_skill_allowed_tools` over the enabled (default agent) or whitelisted (custom agent) skill set. **The always-available framework built-ins (`ALWAYS_AVAILABLE_BUILTIN_TOOL_NAMES` = `read_file`, `review_skill_package`) never count as a business-tool declaration**: a skill whose `allowed-tools` lists only framework tools — or is empty — does not flip the agent into restricted mode. This is folded into `allowed_tool_names_for_skills` itself (not just the lead-agent caller) so the exemption also holds on the subagent path (`subagents/executor.py::_apply_skill_allowed_tools`, which calls the filter without an explicit framework set). Without it, merely *enabling* the built-in `skill-reviewer` (whose only allowed-tool is the always-available `review_skill_package`) strips every other tool from the lead agent **and** its subagents, leaving them with just the framework built-ins. A skill that declares a real business tool still restricts as before. Tests: `tests/test_skill_tool_policy_regression.py`, `tests/test_lead_agent_skills.py`, `tests/test_subagent_executor.py::TestSkillAllowedTools`.
