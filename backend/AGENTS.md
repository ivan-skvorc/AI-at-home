...
- `web_fetch/` - Pluggable `web_fetch` dispatcher (`web_fetch/tools.py:web_fetch_tool`). Backend resolution: `DEER_FLOW_WEB_FETCH_BACKEND` env > tool-config `backend` key > `jina` (default). Optional `fallback` backend tried when the primary returns an `"Error: ..."` string (both-fail reports both). A `github.com` URL that 404s gets an appended private-repo hint (use git-in-sandbox). The shipped `config.example.yaml` points `web_fetch` here with `backend: jina`, so behavior is unchanged by default.
