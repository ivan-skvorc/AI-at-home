...
**Multi-user mode** (fork feature) is a server-wide, runtime-toggleable owner-scoping switch owned by `deerflow/config/runtime_settings.py` (persisted as `runtime_settings.json` under the DeerFlow home dir, read hot with an mtime cache; `config.yaml` is never rewritten). Default `True` = per-user thread isolation (upstream behavior). When `False` the server is one shared workspace: thread listing and per-thread access ignore the owner filter, so every conversation is visible regardless of login (the passwordless "personal server" case, where old per-account histories are otherwise stranded under different ids). The lever is `resolve_owner_scope(value, method_name=...)` — identical to `resolve_user_id` when ON, returns `None` (no owner filter / bypass ownership check) when OFF; **read/access** methods use it while **write/stamp** paths keep `resolve_user_id`, so new rows still record their real owner and re-enabling isolation restores each login's view. Applied in `ThreadMetaRepository`/`MemoryThreadMetaStore` (`search`/`get`/`check_access`/`update_*`/`delete`) and `RunRepository` read helpers (`get`/`list_by_thread`/`list_successful_regenerate_sources`/`list_edit_regenerate_runs`/`get_many_by_thread`); `check_access` returning `True` in shared mode covers every `owner_check=True` route guard at one chokepoint (no `authz.py` change). Toggled via `GET`/`PUT /api/settings/multi-user-mode` (`app/gateway/routers/settings.py`, PUT `require_admin_user`-gated). Security: OFF exposes all conversations to anyone who can reach the server — same trust model as passwordless; keep it ON on shared deployments. Tests: `tests/test_multi_user_mode.py`.
...
**Config Versioning**: `config.example.yaml` has a `config_version` field. On startup, `AppConfig.from_file()` compares user version vs example version and emits a warning if outdated. Missing `config_version` = version 0. Run `make config-upgrade` to auto-merge missing fields. Besides the dict-based `merge_missing`, the upgrade also backfills missing default **tools list entries** (`scripts/config_upgrade.py::backfill_missing_default_tools`): example `tools:` entries whose `name` is absent from the user's list are appended (name-keyed, append-only — existing entries, e.g. a different web_search provider, are never touched), so configs from older bootstraps that skipped defaults regain the out-of-the-box toolset; an appended `bash` entry stays runtime-inactive on the local sandbox unless `sandbox.allow_host_bash` is true. `make doctor` has a matching `default toolset present` check plus a container-sandbox-without-bash-tool warning. When changing the config schema, bump `config_version` in `config.example.yaml`.

**First-run seeding**: every launch path seeds a missing repo-root `config.yaml` from `config.example.yaml` so a fresh checkout starts without a manual `make config` step — `scripts/serve.sh::seed_missing_config` (local `make dev`/`make start`; also seeds `extensions_config.json`, `.env`, and `frontend/.env` from their examples when absent) and the equivalent blocks in `scripts/deploy.sh` (`make up`) and `scripts/docker.sh` (`make docker-start`). Seeding only fires when NO config resolves via `DEER_FLOW_CONFIG_PATH` / `backend/config.yaml` / `config.yaml`, so an existing config is never overwritten; the API-key model auto-config below then uncomments the matching provider block. Tests: `tests/test_serve_config_seed.py`.

**Config Integrity**: `AppConfig.from_file()` loads via `config/yaml_guard.py::safe_load_guarded` — a `SafeLoader` subclass that raises `DuplicateKeyError` (naming the key and both 1-based line numbers) instead of PyYAML's silent last-key-wins, so a config with e.g. two top-level `sandbox:` blocks fails loudly at startup (merge keys `<<:` still work). The same guard runs in the `make dev` regeneration path (`scripts/config_upgrade.py`, invoked by `scripts/config-upgrade.sh`), `scripts/sync-ollama-models.py`, and `scripts/sync-api-key-models.py` so startup refuses before a duplicate could be silently collapsed. All three regenerators are idempotent — an up-to-date config is left byte-identical with no `.bak` churn, and none appends a section that already exists. The **API-key model auto-config** (`scripts/sync-api-key-models.py`, a fork feature run on every launch path right after the Ollama sync) uncomments the model entries between the `# === BEGIN/END auto-model-config: <provider> ===` markers in `config.yaml` when the matching key is present in `.env`; it only ever uncomments (never re-comments), skips a block whose model names are already active (no duplicates vs `make setup` / hand-edits), and no-ops when the key is a placeholder/missing or when the markers are absent. There are eleven independent provider blocks: `anthropic` (direct) and `openrouter` (routed), plus a first-party "home" block for every big-name lab that ships its own API — `openai`, `xai`, `google`, `deepseek`, `mistral`, `moonshot`, `qwen`, `minimax`, `zai` — each gated by that lab's own key (`OPENAI_API_KEY`, `XAI_API_KEY`, `GEMINI_API_KEY`, …). This mirrors how Anthropic is handled: a lab's full lineup lives on its own key while its flagship is also routed via OpenRouter, so each lab's flagship is reachable through both its home API and OpenRouter (blocks never collide because model `name:`s are provider-prefixed). Model definitions are shared with the setup wizard (`scripts/wizard/providers.py`; the `HOME_API_BUNDLES` registry maps each marker slug → gating env var + bundle) and the `config.example.yaml` marker blocks — keep the three in sync. `config/config_lint.py::lint_unknown_config_keys` (called at load, `logger.warning`) flags unknown `sandbox:` keys (difflib "did you mean") and typo-shaped `models:` entry keys (close match to a declared `ModelConfig` field — legitimate provider passthrough stays silent). `scripts/doctor.py` reports duplicate keys and lint findings as proper checks. Tests: `tests/test_yaml_guard.py`, `tests/test_config_integrity.py`, `tests/test_config_upgrade_script.py`, `tests/test_sync_ollama_models.py`, `tests/test_sync_api_key_models.py`.
...
| **Settings** (`/api/settings`) | Fork feature: `GET /multi-user-mode` - read the server-wide multi-user-mode flag; `PUT /multi-user-mode` (admin-only) - toggle it. See "Multi-user mode" below |
...
- `AioSandboxProvider` (`packages/harness/deerflow/community/`) - Docker-based isolation. After a new container passes its readiness check, the provider runs a best-effort session init (`aio_sandbox/git_credentials.py`): it installs a git credential helper inside the container that resolves `GITHUB_TOKEN` from the container environment (forwarded via `sandbox.environment`) at git-invocation time, so the agent can clone private GitHub repos with plain HTTPS URLs and the token never appears in URLs, tool output, logs (container-run env values are redacted), or `.git/config`. `GIT_TERMINAL_PROMPT=0` is defaulted into the container env so credential-less private clones fail fast with the helper's hint instead of hanging on a prompt. Session-init failures are logged and never fail acquisition. Host-run `make dev` runs `scripts/sandbox-preflight.sh` (invoked from `serve.sh`) when config selects this provider: it verifies the container runtime, pulls the sandbox image on first run, and prints a LocalSandboxProvider fallback hint on failure. Tests: `tests/test_aio_sandbox_git_credentials.py`, `tests/test_sandbox_preflight_script.py`. Active-cache and warm-pool entries are checked with the backend during acquire/reuse; definitively dead containers are dropped from all in-process maps so the thread can discover or create a fresh sandbox instead of reusing a stale client. Backend health-check failures are treated as unknown, not dead; local discovery likewise treats an unverifiable container as not adoptable and falls through to create rather than failing acquire. `get()` remains an in-memory lookup for event-loop-safe tool paths — it never touches the ownership store (that would be blocking IO on the event loop); ownership is published on acquire/reclaim and refreshed off the event loop by the dedicated renewal thread (`_renew_owned_leases`). `uses_thread_data_mounts` defaults to backend detection (`LocalContainerBackend=True`, remote/provisioner backends=False), while the optional `sandbox.thread_data_mounts` boolean takes precedence for deployments that guarantee the Gateway and sandbox share the same thread user-data directories. Setting it `true` skips upload-time sandbox acquire/sync; a false positive leaves uploads unavailable to the sandbox. Local-container and hostPath-provisioner mounts use the same stable skill projection roots; PVC-backed skills remain governed by the operator-supplied PVC layout until PVC materialization is implemented. Readiness probes and `agent_sandbox` clients classify loopback/private IPs, single-label cluster hosts, and Docker/Podman internal hostnames as direct control-plane destinations and set `trust_env=False`; external FQDNs and public IPs retain environment proxy support.
  - **Backend selection** (`aio_sandbox/aio_sandbox_provider.py::_create_backend`, precedence: `provisioner_url` > `base_url` > local-container): `provisioner_url` → `RemoteSandboxBackend` (k8s pods); `base_url` → `ExternalSandboxBackend` (`aio_sandbox/external_backend.py`) which binds to ONE externally-managed container (e.g. `make sandbox-up`, `docker/docker-compose.sandbox.yml`) — `create` returns a stable static `SandboxInfo`, `destroy` is a no-op so warm-pool eviction / idle destruction / shutdown never touch the user's container, health is `GET {base_url}/v1/sandbox` over HTTP, and no docker CLI is ever invoked. `session_init_on_discover=True` re-runs the idempotent git-credential setup on discovery (gateway restart / recreated container). `sandbox.request_timeout` plumbs into the `AioSandbox` HTTP client (default 600). `sandbox.mounts` are logged as ignored in external and remote modes; `sandbox.environment` is warned as non-injectable in external mode (set on the container; the token value is never logged). `make sandbox-enable` / `sandbox-disable` (`scripts/sandbox_toggle.py`) rewrite ONLY the `sandbox:` section (text surgery, backup, preserve `environment:`, idempotent, duplicate-key abort); `make sandbox-enable MODE=container` (`enable --mode container`) writes the per-thread local container block (no `base_url`, so `/mnt/user-data` is host-backed — the mode for clone-and-debug workflows) instead of the external block, and pins `image: ghcr.io/agent-infra/sandbox:1.11.0` (the same working image `docker/docker-compose.sandbox.yml` uses) so local-container mode does not fall back to the provider's broken `:latest` `DEFAULT_IMAGE`; the setup wizard (`scripts/wizard/writer.py`) and `make config` (`scripts/configure.py`, which calls `sandbox_toggle.py`) write the same pinned block. `scripts/sandbox-preflight.sh` external path probes `base_url` and `docker compose -f docker/docker-compose.sandbox.yml up -d` + health-polls when unreachable, failing fast with `make sandbox-logs` / `sandbox-disable` hints. `scripts/check.py` adds advisory Docker diagnostics (daemon, `docker` group, compose, port 8091; fish-compatible Arch+Debian remediation, never fails, never runs privileged). Tests: `tests/test_external_sandbox_backend.py`, `tests/test_sandbox_toggle.py`, `tests/test_sandbox_preflight_script.py`.
  - **Per-command timeout & debug run-time options (local container mode)**: `sandbox.bash_command_timeout` (the LocalSandbox host-bash budget) is also forwarded to the AIO sandbox per command — the idle (no-new-output) `no_change_timeout` on the persistent-shell path and the wall-clock `hard_timeout` on the env-bearing `bash.exec` path (`AioSandbox.execute_command(..., timeout=...)`, both the primary and ErrorObservation-retry calls). Falls back to the 600s defaults when unset; a one-shot warning fires when the command timeout exceeds `request_timeout` (the HTTP client would abort first). `sandbox.expose_ports: [int]` publishes extra container ports 1:1 to the host loopback (`_resolve_docker_bind_host()`), so a program the agent runs is reachable from the host browser; fixed host ports, so one container per port. `sandbox.extra_capabilities: [str]` adds `--cap-add` flags (Docker runtime only — e.g. `SYS_PTRACE` for gdb/strace attach; warned-and-skipped on Apple Container). Both are wired `_load_config` → `_create_backend` → `LocalContainerBackend._start_container`, and both are warned-as-ignored in external/provisioner modes (`_warn_ignored_container_options`) — declare the equivalent `ports:` / `cap_add:` in `docker/docker-compose.sandbox.yml` for external mode. Tests: `tests/test_aio_sandbox.py` (timeout plumbing), `tests/test_aio_sandbox_local_backend.py` (ports/caps in the docker run command), `tests/test_aio_sandbox_provider.py` (config wiring + ignore warning).
  - **Host-run Ollama reachability**: sandbox containers reach a host-run Ollama out of the box. Both the external compose file (`docker/docker-compose.sandbox.yml`) and `LocalContainerBackend._start_container` map `host.docker.internal:host-gateway` (Linux daemons don't provide the alias; Docker runtime only — Apple Container is skipped) and default `OLLAMA_HOST=http://host.docker.internal:11434` into the container env (an explicit `sandbox.environment` entry / `DEER_FLOW_SANDBOX_OLLAMA_HOST` compose override wins). Because a loopback-bound host Ollama (its default) still refuses bridge-gateway connections, `scripts/sandbox-preflight.sh::warn_if_host_ollama_unreachable_from_containers` prints an advisory with the `OLLAMA_HOST=0.0.0.0` fix after a successful preflight (never fails; quiet when reachability is undeterminable or on Docker Desktop, which proxies host loopback), and `scripts/sync-ollama-models.py --container` (`container_ollama_warning`) prints the same remediation when the written `base_url` isn't reachable from containers. Tests: `tests/test_sandbox_compose_ollama.py`, `tests/test_aio_sandbox_local_backend.py`, `tests/test_sandbox_preflight_script.py`, `tests/test_sync_ollama_models.py`.
...
- `web_fetch/` - Pluggable `web_fetch` dispatcher (`web_fetch/tools.py:web_fetch_tool`). Backend resolution: `DEER_FLOW_WEB_FETCH_BACKEND` env > tool-config `backend` key > `camoufox` (code-level default when no `backend` key is set). Optional `fallback` backend tried when the primary returns an `"Error: ..."` string (both-fail reports both). A `github.com` URL that 404s gets an appended private-repo hint (use git-in-sandbox). Camoufox is the default everywhere: the shipped `config.example.yaml` points `web_fetch` here with `backend: camoufox`, and an entry that omits `backend` gets camoufox too — set `backend: jina` explicitly to use the cloud reader. `scripts/detect_uv_extras.py` mirrors this resolution (explicit `backend:`/`fallback:`/`use:` selection, a dispatcher entry without a `backend` key, or `DEER_FLOW_WEB_FETCH_BACKEND=camoufox`) so the extra auto-installs in every matching case.
- `camoufox_fetch/` - Local, key-less, JS-capable `web_fetch` backend (`fetch_url_via_camoufox`). `browser.py` lazily launches ONE shared headless `AsyncCamoufox` (reused across requests; `asyncio.Lock`-guarded; fresh page per request always closed; clean `shutdown()` + atexit). Renders the page, extracts readable markdown (offloaded via `asyncio.to_thread`), falls back to body inner_text, caps content. `camoufox` is imported lazily (optional `camoufox` uv extra); failures return actionable strings (`uv sync --extra camoufox` / `make fetch-browser`) — browser-install state is probed via camoufox's `version.json`. **Auto-install across all launch paths:** `scripts/detect_uv_extras.py` emits `--extra camoufox` when config selects it (so the package installs only when chosen), and `scripts/ensure_camoufox.py` fetches the browser binaries — idempotent + best-effort, mirroring `browser._camoufox_browser_present`'s `version.json` probe so it no-ops when already present. It runs after `uv sync` in `scripts/serve.sh` (local) and `docker/dev-entrypoint.sh` (Docker dev, persisted in the `gateway-camoufox` volume); the prod image bakes the browser in `backend/Dockerfile`'s builder stage and copies `~/.cache/camoufox` into the runtime stage. `make fetch-browser` stays as a manual pre-download. Tests: `tests/test_web_fetch_dispatcher.py`, `tests/test_camoufox_fetch.py`, `tests/test_ensure_camoufox.py`.
- `tavily/` - Web search (5 results default) and web fetch (4KB limit)
- `jina_ai/` - Web fetch via Jina reader API with readability extraction. `tools.py` exposes `fetch_url_via_jina` (plain importable callable reused by the dispatcher) plus the backward-compatible `web_fetch_tool`.
...
The default `web_search` in `config.example.yaml` is `searxng/` (self-hosted instance bundled with the Docker stacks; `docker/searxng/settings.yml` enables the JSON API and disables the bot limiter). Its `base_url` resolution is `DEER_FLOW_SEARXNG_BASE_URL` env var > tool config `base_url` > `http://localhost:8088` — docker-compose sets the env var to the in-network `http://searxng:8080` so one `config.yaml` works both host-run and containerized (same pattern as `DEER_FLOW_CHANNELS_*`). The launch scripts (`deploy.sh`, `docker.sh`, `serve.sh`) call `scripts/detect_searxng.py` before starting services: it prints `skip` (config doesn't use the provider), `external <url>` (a reachable instance already exists — the scripts export `DEER_FLOW_SEARXNG_BASE_URL` and don't start the bundled service; for Docker contexts, host-local instances are verified reachable from containers via the bridge gateway / Docker Desktop), or `bundled` (the scripts start the bundled container). Tests: `backend/tests/test_detect_searxng.py`.
