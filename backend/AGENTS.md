...
- Scheduled background runs are intentionally non-interactive: the lead-agent toolset excludes `ask_clarification` when `context.non_interactive=true`. That key is honored only for internally-authenticated callers (the scheduler launch path); a client-supplied `context.non_interactive` is dropped. `scheduler.queue_timeout_seconds` bounds the durable wait; do not reintroduce skip-on-overlap or count waiting rows against `max_concurrent_runs`.
