...
### Changed
- **upstream:** merged `upstream/main` (46 commits since 2026-09-22). Brings route
  permission checks on create/run/memory/agent routes with per-user `USER.md`, SearXNG
  pagination for `max_results` above one page, plugin manifests and static asset
  directories, request-scoped run evidence for extensions, and MCP sessions scoped by
  thread incarnation. No config keys were added, so `config_version` is unchanged.
- **uploads:** deleting an uploaded document now removes only that file, per upstream
  (#5673). The fork's extra cleanup of the `<name>.<ext>.md` companion went with it, so
  both converted Markdown files stay listed and can be deleted on their own.
- **upstream:** merged `upstream/main` (136 commits since 2026-09-15). Brings upstream's
  ...
  default), administrator-managed shared models under Settings → Models, full-stack plugin
...
- **sandbox:** `sandbox.request_timeout` no longer needs to be raised alongside
  ...
  retired as factually wrong. The setting still applies to the calls that budget does not
  reach — session creation and the file/list operations.

### Fixed
- **web_search:** a SearXNG engine block that lands on a later page no longer throws away
  the results earlier pages returned. Carried over unchanged into upstream's pagination, the
  fork's blocked-engines check would have turned a blocked page 2 into
  `SearxngEnginesUnavailableError`; only a first page with nothing to show raises.
- **sandbox:** a non-positive per-call command timeout is treated as unset again. Upstream's
  ...
  aborted almost immediately with its outcome reported as unknown.
- **sandbox:** the remote bash path no longer sends a per-call timeout. The provider is
  already built with `bash_command_timeout` as its default, and sending it twice let one
  setting arrive by two routes.
- **web_fetch:** a `web_fetch` entry with no `api_key` of its own no longer borrows the
  `web_search` entry's key, which belongs to whichever provider is configured there — a
  Serper key could reach Tavily. It falls back to `TAVILY_API_KEY` instead.
...
- **repo:** **Merged `bytedance/deer-flow@6469833`** (41 commits) into the fork.
  ...
  opt-in task notes with compacted-history recall, elision of superseded
  `write_file` payloads from model-bound requests, occurrence-status filters on
  scheduled-task run history, an OpenAI-compatible image-generation provider, a
  configurable Gateway `recursion_limit`, and a long run of sandbox, MCP,
  runtime and Windows-portability fixes. Four resolutions are worth naming
  because a reviewer cannot see them in the diff:
  - **Upstream made the run budget configurable and set it to `100`; this fork
    keeps `250`.** A run's `recursion_limit` caps LangGraph super-steps, and a
    clone-and-iterate session exceeds a lower cap and aborts mid-task with
    `GraphRecursionError`. Taking upstream's number would have been green in
    every test and wrong only in production, so the value now lives in three
    places that must agree — `AppConfig.recursion_limit`, the config-load
    fallback in `app/gateway/services.py`, and `config.example.yaml` — and
    `tests/test_recursion_limit_default.py` pins all three together.
  - **Alembic split into two heads a third time.** Upstream hung
    `0023_user_preferences` off `0022_scheduled_occurrence_seq`, the revision
    `0023_merge_pricing_scheduler` had already claimed. `0024_merge_preferences`
    is a third no-op merge revision naming both heads as parents — again
    deliberately *not* a re-parenting, which would leave a database stamped at
    one branch reading as "at head" with the other branch's DDL never applied.
    `tests/test_migration_merge_points.py` now guards the new branch root too.
  - **Account preferences and per-conversation context wanted the same three
    fields.** Upstream's new sync uploads `model_name` / `mode` /
    `reasoning_effort` as account defaults; this fork scopes all three per
    conversation (§ thread context), so picking a model in one chat must not
    become the default for every other chat on every device. `updateThreadSettings`
    therefore writes only the thread's own override for `context` and never calls
    the preference hook, while `resolveThreadContext` refreshes only keys a
    conversation has *already* overridden. The fork's `democracy` mode was also
    missing from both preference schemas, where a narrowed union drops the value
    silently on the frontend and 422s on the backend; both now carry it.
  - **The agent-guidance chain went 277 bytes over its hard limit.** The merge
    added roughly 750 bytes across four files that each stayed inside their own
    budget — the exact shape the chain check exists for. Prose in
    `backend/AGENTS.md` and `agents/middlewares/AGENTS.md` was compressed rather
    than cut, and no instruction was dropped.
- **repo:** **Merged `bytedance/deer-flow@0464502`** (49 commits) into the fork.
  Upstream brings custom-skill export with a revision-bound preview, paginated
  thread-run history and scheduled-task run history, an `interval` schedule type
  and cron-occurrence previews, opt-in parent context snapshots for subagents,
  in-place extension upgrades and constructor kwargs for config-declared
  middlewares, elision of blocked write payloads from model-bound requests,
  effective-permission gating of the delete/cancel UI, bounded document outlines
  and previews, and a long run of sandbox, scheduler, channel and Windows-portability
  fixes. Three resolutions are worth naming because a reviewer cannot see them in
  the diff:
  - **Alembic had two heads again — and for a new reason.** The first collision
    (`0022_merge_pricing_projects`) merged the fork's `0019_runs_pricing_snapshot`
    into upstream's `0021_batch_acceptance`. Upstream has now grown
    `0019_thread_incarnations` → `0022_scheduled_occurrence_seq` off that *same*
    parent, so `0021_batch_acceptance` has two children and the tree split again.
    `0023_merge_pricing_scheduler` is a second no-op merge revision naming both
    heads as parents — again deliberately *not* a re-parenting, which would leave
    a database stamped at one branch reading as "at head" with the other branch's
    DDL never applied. Expect a merge revision on every sync where upstream
    extends a revision a merge point already claimed.
  - **The subagent acceptance-checklist prose was moved, not merged.** Upstream
    roughly doubled that paragraph in `subagents/AGENTS.md` and added a Windows
    path-portability one. Both now live in `subagents/ACCEPTANCE_CHECKS.md`, which
    the guide summarizes and points at; the guidance chain that file sits in was
    329 bytes under its hard limit before this sync and is 528 under it now.
  - **`config_version` bumped to 52.** Upstream added `read_before_write`'s
    `elide_blocked_payloads` / `elide_min_chars`, the `{class, kwargs}` middleware
...
- **repo:** **Merged `bytedance/deer-flow@a3848ef`** (27 commits) into the fork.
  ...
  permissions on `GET /auth/me`, a Sofya web-search provider, `summary_text` on
  embedded-client `values` events, acceptance checks on durable batch items, and a
  run of sandbox, doctor and compaction fixes. Four resolutions are worth naming
  because a reviewer cannot see them in the diff:
  - **Alembic had two heads and would have refused to start.** The fork's
    `0019_runs_pricing_snapshot` and upstream's `0019_projects` both descend from
    `0018_oauth_identity_pg_partial`. `0022_merge_pricing_projects` is a no-op merge
    revision naming both as parents — deliberately *not* a re-parenting, which would
    leave a database already stamped at one branch reading as "at head" with the
    other branch's tables never created.
  - **One sidebar chat row, not two.** Upstream extracted a `ThreadSidebarItem`
    from `RecentChatList` for its project groups; the fork had rewritten the same
    component around chat folders. Rather than keep a copy per list, the row is now
    a single shared component and the folder submenu (§32) rides in as an optional
    node, so **Move to project** and **Move to folder** are both reachable from
    every chat row.
  - **Two `createThread`s became one.** Both sides added an exported function of
    that name to `core/threads/api.ts`, with different signatures. The unified
    options-object form serves both callers, and its wire shape is now pinned.
  - **`config_version` bumped to 51.** Upstream added four `sandbox.*` fields to
    `config.example.yaml` without bumping it, so `make config-upgrade` would never
    have delivered them to an existing install.
- **docs:** The API-key model auto-config's OpenRouter lineup is described correctly
  in `scripts/sync-api-key-models.py` and `README.md`. Both prose copies still said
