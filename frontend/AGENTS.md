...
The workflow selection (fork feature) — lead model, subagent model, `mode`, and `reasoning_effort` — is **per conversation**, so concurrent chats can run different local/cloud models independently. `core/settings/local.ts` persists a per-thread override under `deerflow.thread-context.<threadId>` holding just those `THREAD_SCOPED_CONTEXT_KEYS` (`pickThreadScopedContext` filters out route-derived `agent_name`), and `applyThreadContextOverride` layers it on the shared base settings. `core/settings/store.ts::updateThreadSettings("context", …)` writes **only** that thread's override and never the shared `deerflow.local-settings` blob — writing the model there is exactly what leaked a selection into every other conversation that had not explicitly picked one (and, via the cross-tab `storage` event, into other browser tabs). `useThreadSettings(threadId)` reads base settings + that override; snapshots keep a stable object reference so changing one thread's selection does not re-render another thread's hook. Legacy `deerflow.thread-model.<threadId>` (model-name-only) pins are migrated on read. A brand-new conversation has no override and so falls back to the app default model via `InputBox`'s auto-select effect (agent default → `models[0]`), rather than inheriting whatever was last picked elsewhere. The sidecar's second conversation keeps its own in-memory `sidecar.context`, independent of this persisted store.

The model-dropdown **sort/group preference** (fork feature) is the opposite scope: it is **per browser, shared across threads**, so it lives in the shared `deerflow.local-settings` blob (`core/settings/local.ts`, `modelPicker` = `{ sortKey, sortDir, groupByProvider }`) rather than a per-thread override — how you like the list ordered is not a property of one conversation. Price and provider are parsed from `display_name` (they are not on `/api/models`) in `core/models/sorting.ts`; the shared `components/workspace/model-picker-controls.tsx` (`ModelPickerControls` + `ModelPickerList` + `ModelDisplayName`) renders the controls, the ordered/grouped list, and each name's colour-coded price for all three pickers (lead + subagent in `input-box.tsx`, sidecar in `sidecar/sidecar-panel.tsx`). `splitModelNamePriceSegments` is the pure splitter behind `ModelDisplayName`: it paints a price green, and a discounted `($list → $promo*)` name red-then-green (list, then the promo you actually pay), matching the cost overview's colours. It is total — an unparseable name renders verbatim as one text segment and the segments always rejoin to the original string, so colouring can never drop characters from a model name. `ModelDisplayName variant="compact"` is the width-capped **trigger** form: `compactModelDisplayName` drops the provider suffix (keeping `(p)`), and only the leading model name may ellipsize so the price pair is never cut. That form requires `w-full` on the host `ModelSelectorName` — it sits in a `flex-col items-start` container where `flex-1` sizes the cross axis (height), so without it the span is `fit-content`, overflows the capped button, and its own `truncate` never fires. Keep `w-full` on all three triggers. See FORK.md §8.
...
- **Reduced motion** — new decorative/continuous animations must honor the reduced-motion preference. For canvas/WebGL or JS-driven effects (e.g. the landing `Galaxy`/`FlickeringGrid`, the subagent `ShineBorder`/ambilight), skip rendering when `useReducedMotion()` (from `@/core/appearance`) is true. For pure-CSS decorative animations, neutralize them under both `@media (prefers-reduced-motion: reduce)` and `:root[data-reduce-motion="true"]` in `styles/globals.css` (the root attribute is set by `components/reduce-motion-effect.tsx`). Leave functional status indicators (spinners, streaming dots) animated.
...
Reaching a dev server on anything other than localhost — a LAN address, or a proxied hostname — needs that host in Next's `allowedDevOrigins`, which gates `/_next/*`, fonts, and HMR. Without it those requests 403 and the page renders server-side but never hydrates, so nothing on it responds. `src/dev-origins.js::getAllowedDevOrigins()` (wired in `next.config.js`) builds that list. This fork **defaults it to the private-LAN + Tailscale ranges** (`10.*`, `172.*`, `192.168.*`, `100.*` Tailscale CGNAT, `**.ts.net`, `*.local`) so `make dev` reached from a phone on the network / over Tailscale hydrates out of the box (matching the passwordless-for-LAN default). `DEER_FLOW_DEV_ALLOWED_ORIGINS` (comma-separated; a full URL is reduced to its host) adds hosts the defaults miss — a custom domain, an IPv6 literal — and `DEER_FLOW_DEV_ALLOWED_ORIGINS_STRICT=1` drops the built-in defaults for upstream's stricter behavior. Next's `matchWildcardDomain` splits on `.` and matches from the right (`*` = one segment, `**` = trailing segments), so the IP patterns match octet-wise; `tests/unit/dev-origins.test.ts` runs the fork defaults through Next's real matcher. Development only; production builds ignore `allowedDevOrigins`.
