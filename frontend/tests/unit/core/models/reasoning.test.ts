...
describe("getResolvedMode", () => {
  it("clamps only thinking mode to flash on a model without thinking", () => {
    // Fork rule (FORK.md): pro/ultra/democracy stay selectable; see
    // tests/unit/core/models/capabilities.test.ts for the regression.
    expect(getResolvedMode("thinking", legacyPlain)).toBe("flash");
    expect(getResolvedMode(undefined, legacyPlain)).toBe("flash");
    expect(getResolvedMode("pro", legacyPlain)).toBe("pro");
  });
