...
describe("interaction-only bundle boundaries", () => {
  ...
  it("loads each settings page from its active section", () => {
    ...
    // Upstream ships 10 settings pages; this fork adds three more (the
    // follow-up Suggestions page, the System Prompt editor, and the automatic
    // Conversation Titles page). Every one must still be a dynamic import.
    expect(dialog.match(/dynamic\(/g)).toHaveLength(13);
