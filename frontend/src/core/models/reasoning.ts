...
const MODE_EFFORT_PRESETS: Record<InputMode, GenericReasoningEffort> = {
  ...
  pro: "medium",
  ultra: "high",
  // Democracy (fork feature) runs each panelist at Ultra's depth.
  democracy: "high",
};
 ...
 *
 * Fork rule: on a model without thinking only `thinking` itself falls back to
 * `flash`. Pro, Ultra and Democracy also switch on plan mode and subagents,
 * which work on any model, and the backend already drops thinking for a model
 * that lacks it — clamping them too locked every cloud model (which defaults
 * to `supports_thinking: false`) out of them.
 */
export function getResolvedMode(
  ...
  if (thinking === "unsupported") {
    return mode === "thinking" || mode === undefined ? "flash" : mode;
  }
