#!/usr/bin/env python3
"""Install (or remove) a systemd **user** timer that runs the Camoufox +
SearXNG auto-update loop (``scripts/update_camoufox_searxng.py``) daily **and
on boot**.

This is the "automatically ran update loop": a ``systemd --user`` timer that
fires shortly after boot (``OnBootSec``) and once a day (``OnCalendar=daily``),
with a randomized delay and ``Persistent=true`` so a missed run catches up
after downtime, and runs the updater inside the backend virtualenv via
``uv run``. The boot trigger means a machine that is powered off at the daily
time — or freshly booted — refreshes both components on startup instead of
waiting for the next calendar slot. systemd is the idiomatic scheduler on this
fork's target (Arch / CachyOS), and a *user* timer needs no root.
...
installation prints the equivalent ``cron`` lines instead of failing — a daily
entry plus a ``@reboot`` entry that covers the on-boot run.
...
def timer_unit() -> str:
    """systemd .timer content — daily + on boot, jittered, catches up missed runs."""
    return f"""\
[Unit]
Description=DeerFlow: Camoufox + SearXNG auto-update (daily + on boot)
Documentation=file://{REPO_ROOT / "FORK.md"}

[Timer]
OnCalendar=daily
# Also run shortly after the machine boots, so a PC that was powered off at the
# daily time (or was just provisioned) refreshes both components on startup
# instead of waiting for the next calendar slot.
OnBootSec=2min
# Spread the run out so a fleet of machines does not hammer the registries at
# the same instant (at midnight, or all at once after a shared reboot / power
# outage), and run a missed timer after the machine was off.
...
def cron_reboot_line() -> str:
    updater = REPO_ROOT / "scripts" / "update_camoufox_searxng.py"
    backend = REPO_ROOT / "backend"
    uv = _uv_path()
    # The cron equivalent of the timer's OnBootSec: run once each boot, quiet on
    # success. Pairs with cron_line()'s daily entry.
    return f"@reboot  cd {backend} && {uv} run python {updater} >/dev/null 2>&1"
...
def install() -> int:
    if not _systemctl_user_available():
        ...
        print("Add these lines to your crontab instead (`crontab -e`) for a daily run plus a run at boot:", file=sys.stderr)
        print(f"\n    {cron_line()}\n    {cron_reboot_line()}\n", file=sys.stderr)
        return 1
    ...
    print("✓ Enabled and started the auto-update timer (daily + on boot).")
    ...
    print("  Tip: `loginctl enable-linger` runs the boot + daily timer even when you are logged out.")
    return 0
