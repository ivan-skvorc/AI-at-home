...
def build_minimal_config(
    *,
    ...
    ollama_system_ram_gb: float | None = None,
    ...
    if ollama_vram_gb and ollama_vram_gb > 0:
        # Consumed by scripts/sync-ollama-models.py on every launch: sizes each
        # auto-populated Ollama entry's num_ctx to the GPU memory budget.
        ollama_section: dict[str, Any] = {"vram_gb": ollama_vram_gb}
        if ollama_kv_cache_type:
            ollama_section["kv_cache_type"] = ollama_kv_cache_type
        if ollama_system_ram_gb and ollama_system_ram_gb > 0:
            ollama_section["system_ram_gb"] = ollama_system_ram_gb
        data["ollama"] = ollama_section

    header = f"# DeerFlow Configuration\n# Generated by 'make setup' on {today}\n# Run 'make setup' to reconfigure, or edit this file for advanced options.\n# Full reference: config.example.yaml\n\n"
...
def write_config_yaml(
    ...
    *,
    ...
    content = build_minimal_config(
        ...
        ollama_system_ram_gb=ollama_system_ram_gb,
