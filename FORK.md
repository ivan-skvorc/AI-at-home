...
### 17. Explicit `price:` and `discount:` fields, with an expiry

The cost display was driven by two *implicit* sources: a `pricing:` block, and
failing that, a `($in/out)` pair parsed out of `display_name`. Both work, and
both stay supported — but neither is a field an operator can see and set, which
is how a model ends up silently unpriced and the header renders `—`.
...
Precedence is `price:` > `pricing:` > the display name, so adding `price:` to a
model is always safe and always wins.

**The discount is additive and never billed against.** Spend is estimated at the
standard rate and the discount is shown beside it, because a promotion can end
at any time and an under-estimate is worse than a slightly high one. This is the
same rule the existing `promo_*` rates follow — `discount:` is a shorter
spelling of it that also carries an expiry.

**The expiry is enforced once, at the bottom.** An expired discount is dropped in
`build_pricing_map`, so it never reaches a `ModelPricing` at all. Every consumer
— the chat header, the spend page, `SpendBudgetMiddleware`, `GET /api/models` —
is correct without repeating the check, and there is no second place for the two
to disagree. Do not add a downstream expiry check; that is how they drift.

**Two unknowns both resolve to "expired", deliberately.** An `until` that cannot
be parsed, and a run where the current time is unavailable. The alternative is
"assume it's still valid", which reintroduces the exact bug the field removes —
[the audit section](#auditing-the-model-list-settings--pricing) names it: an
expired promo leaves the header advertising a discount nobody is getting.
Over-stating cost is corrected by the provider's bill; the other direction is
silent. A discount with **no** `until` is unaffected by an unknown clock — there
is nothing a date could tell us — so the common case is not punished for a
problem it does not have.

`GET /api/models` now returns the resolved `price` (discount already
expiry-filtered), so the model dropdown reads a field instead of parsing a label
and cannot advertise a stale starred pair from a `display_name` nobody updated.
`core/models/sorting.ts::resolveModelPrice` prefers it and falls back to the
name, so an unpriced or hand-added model behaves exactly as before.

One trap worth stating: `ModelConfig` is `extra="allow"`, so `price` and
`discount` **must** stay in the model factory's exclude set. An unexcluded key is
forwarded into the provider client's constructor and from there into the
completion request payload — a cost annotation would become a malformed API
call. `pricing` and `fallback` are excluded for the same reason.
...
- [ ] **Every configured discount carries an `until`.** A `discount:` block without one is a *permanent* claim: it keeps being applied, and shown, until a human edits the file — which is the exact failure the expiry field exists to remove, just moved one step later. The audit's `promo_ended` finding tells you a provider's promotion is over; an `until` means the config stops advertising it whether or not anyone reads that issue. Check the shipped example and your own config:

  ```bash
  # every `discount:` block should be followed by an `until:` within a few lines
  grep -n -A 6 '^\s*discount:' config.example.yaml | grep -c 'until:'
  ```

  An expiry that has already passed is *not* a failure here — that is the mechanism working, and the entry is inert until someone refreshes it. Removing the stale block is tidying, not a fix. Note the two fail-closed cases while you are in here, because both look like "the discount vanished": an `until` that cannot be parsed, and a run where the current time is unavailable, are both treated as expired rather than eternal. Pinned by `backend/tests/test_model_price_fields.py::TestDiscountExpiry`.
...
| **Explicit `price:` / `discount:` fields** (§17) | `cd backend && uv run pytest tests/test_model_price_fields.py` (34 tests) covers the field precedence (`price:` > legacy `pricing:` > the `($in/out)` pair in `display_name`), the additive discount, and the expiry rules; `cd frontend && pnpm test sorting` covers `resolveModelPrice` preferring the server figure. Wiring: `deerflow/pricing.py` (`parse_discount_expiry`, `_raw_from_price_fields`, `_resolve_discount_window`), the `price`/`discount` fields on `ModelConfig`, the **factory exclude set** in `models/factory.py`, `ModelPriceResponse` on `GET /api/models`, and `core/models/sorting.ts::resolveModelPrice`. **Three properties must not be "fixed" into their opposites:** an expired discount is dropped in `build_pricing_map`, so it never reaches a `ModelPricing` and no consumer re-checks the window — do not add a second expiry check downstream; an unparseable `until` and an unavailable clock both mean *expired*, never *eternal*; and `price`/`discount` must stay in the factory's exclude set, because `ModelConfig` is `extra="allow"` and an unexcluded key is forwarded into the provider client and from there into the completion request payload. Manual: set a `discount:` with `until:` in the past and confirm the header shows only the standard rate. |
