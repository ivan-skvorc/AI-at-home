...
- **Upstream rewriting an ordering the fork's own flow depends on, with no
  conflict anywhere.** The 2026-09-12 sync replaced message ordering with a
  trusted-position skeleton (`frontend/src/core/threads/message-order.ts`) and a
  local-turn anchor. Nothing in the fork conflicted, every unit test stayed
  green, and Gaslight mode's prompt-edit replay (§18) started rendering the
  branched thread's inherited turns **after** the new human instead of before
  it — the version switcher then has no anchor and never appears. Only
  `edit-message-versions.spec.ts` catches it, which is why the e2e row says to
  run the whole suite. Diagnosing one of these means comparing against
  `origin/main` on the same commit **and the same browser build** before calling
  it a regression, then reading the anchor (`preSubmitMaxSeq`, the confirmed
  pre-submit set) rather than the component.
