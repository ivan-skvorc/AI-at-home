...
- [ ] **`AGENTS.md` byte budgets** — `backend/tests/test_agent_guidance_check.py` asserts every guidance file is at or under a **soft** budget (root 16 KiB, module 28 KiB, local 40 KiB, chain 80 KiB), and in pytest it is a hard assert, not a warning — `scripts/check_agent_guidance.py`, the standalone CI job, only *warns* below the hard limits (20/32/48/96 KiB) because it is run without `--strict-warnings`, so `make test` is the gate that actually stops you. The root file runs close to the line (38 bytes of headroom at the time of writing; `backend/AGENTS.md` had 715), so *documenting a feature can fail CI on its own*. When a section does not fit, push the depth down to the nearest local guide — which has a 40 KiB budget and sits beside the code anyway — and leave a one-line pointer in the module file, the way `models/AGENTS.md` and `agents/AGENTS.md` already carry pricing and prompt detail. Check before pushing:
