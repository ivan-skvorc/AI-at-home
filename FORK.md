...
- **Two Alembic revisions claiming the same parent.** The fork's
  ...
  migration — **and again whenever upstream extends a revision a merge point already
  claimed**: the 2026-09-12 sync grew `0019_thread_incarnations` →
  `0022_scheduled_occurrence_seq` off `0021_batch_acceptance`, splitting the tree a second
  time, and the answer was a second merge revision (`0023_merge_pricing_scheduler`), not a
  re-parent. The depth is in
...
- **Upstream starting to depend on a field the fork's _test fixture_ had been
  faking.** The 2026-09-12 sync's new ordering repair
  (`frontend/src/core/threads/message-order.ts` + `restoreLocalTurnMessageOrder`)
  reads `run_id` to tell the current turn's steps from earlier ones. The e2e mock
  had always stamped one `run_id` per *thread*; the real feed stamps one per
  *turn*. No conflict, no unit-test failure — and Gaslight mode's prompt-edit
  replay (§18) started rendering a branched thread's inherited answer **below**
  the turn replayed into it, which silently removes the version switcher. Two
  lessons, in order of how much time they save: **the mock is part of the
  contract**, so when a merge makes one spec fail, ask whether the fixture models
  what the backend actually sends before reading the product as broken; and
  compare against `origin/main` on the same commit **and the same browser build**
  before calling anything a regression. `git grep -n run_id
  frontend/tests/e2e/utils/mock-api.ts` is the check.
...
- **Upstream shipping a config-upgrade migration under its own version number.** The
  fork's `config_version` runs ahead of upstream's, so fork configs were stamped past
  upstream's number before upstream reached it, and a migration ported under that key
...
- [ ] **VPN-hostile defaults** (§31): `cd backend && uv run pytest tests/test_searxng_client.py tests/test_ensure_camoufox.py tests/test_camoufox_runtime_deps.py tests/test_search_fetch_defaults.py -q`. Each silent: `TestCamoufoxRuntimeLibraries::test_the_libraries_are_installed_beside_the_browser_copy` (a browser copied where it cannot run passes every presence check); `TestFetchEnvironment::test_the_fetch_call_site_actually_uses_the_scrubbed_environment` (the *call site*, not the helper — the helper tests pass against a module that cannot resolve `os`); `TestUnresponsiveEngines::test_all_engines_blocked_raises_rather_than_returning_empty` **with** `test_a_genuinely_empty_result_set_is_still_a_success` ("empty means error" breaks a no-match search) and `test_a_blocked_later_page_keeps_the_results_already_collected` (the client walks pages; only page 1 may raise); and `TestSearxngEngineMix` (the VPN-tolerant engines, and that the json/limiter deltas survived the edit). A sync that drops the apt block from `backend/Dockerfile`, or takes upstream's `_search_page` without the `unresponsive_engines` branch, is green elsewhere.
...
- [ ] **Changed a shared UI control? Run the whole e2e suite, not the one spec you thought of.** `pnpm test:e2e` with no filter. A control's _specs_ are not the specs in the file you edited: they are every spec that clicks that control anywhere in the app, and nothing in `pnpm check` or `pnpm test` knows the difference. Observed live on the model-picker unification — the composer's picker was swapped into five screens, the Democracy spec was found and fixed by hand, and `suggestions-settings.spec.ts` was missed entirely because it drives the _same control_ from a different page. It failed only in CI. Two rules fell out of it, both now pinned by `frontend/tests/unit/components/workspace/model-picker-sites.test.ts`: a spec must locate a shared control by that control's **own** `data-slot`, never by the ARIA role of the primitive underneath (swap the primitive and the locator silently matches nothing), and a fast unit test should assert the contract so a six-minute e2e job is not the thing that tells you.
  ...
  A handful of specs (artifact streaming, the Lark integration, reasoning order) are **flaky** on an older browser build; on `chromium-1194` all three, plus `subtask-card.spec.ts`'s two *running*-state cases, fail **every** run — on an untouched `origin/main` as readily as on a branch. `thread-ordering.spec.ts`'s "a live compaction during submit" case is the intermittent member of the same family (observed 1-in-2 on both, same sandbox, same commit). Reproduce the baseline with a worktree whose `node_modules` is **hard-linked** (`cp -al`), not symlinked: Turbopack refuses a symlink pointing out of the project root and the build fails before a spec runs, which reads like a broken baseline. Confirm against CI on the same commit before calling one a regression, or reproduce on `origin/main` in the same sandbox: a spec CI passes and the sandbox fails is the browser, not your change. And check that your local `main` is not stale before using it as the baseline — a spec added upstream since your last fetch simply does not exist there, so it "passes" by not running, which reads exactly like your branch broke it.

  **A flake with a cause is fixed, not retried.** `thread-history.spec.ts`'s follow-ups were the other kind: the list is virtualized, so a message is only in the DOM while inside the window around the current scroll position, and asserting immediately after a submit raced the history-page refetch that re-renders that window. It failed about half of local runs and lost all three CI attempts on 2026-09-01, on a PR that had not touched it. `expectMessageAtBottom` re-scrolls to the bottom and retries, which keeps what the assertion is for. If those assertions start flaking again the race is back — find it rather than widening the timeout. §28 fixes the scroll-ownership half of that race and explains why it still cannot be pinned by a red-then-green e2e.

  **An upstream e2e can assume upstream's mount lifecycle, which the fork has changed.** Seen live on the 2026-09-03 sync: upstream PR #5045 added `thread-title-sync.spec.ts`, whose "a stale metadata response cannot restore the old title after rename" case produces its racing in-flight metadata request by navigating away to the chat list and back — relying on the thread page **remounting** to refetch. Upstream has no workspace-level live chat slots (§9); this fork keeps the thread's `<ChatInstance>` and its `useThreadMetadata` query mounted across that navigation (`syncRoute` re-selects the same slot without a remount), so no refetch fired, the test's `staleMetadataRequestStarted` gate never resolved, and it hung to a 30 000 ms timeout — green on upstream CI, red only once merged here. It is invisible to `pnpm check`/`pnpm test` and, being a title race, does not surface in the one spec file you would think to run. The fix was **not** to touch the test (loosening it would drop the invariant) but to close the real gap the fork's feature opened: a kept-alive thread never refreshed its metadata after first load, so an externally-changed title/goal stayed frozen until a full reload. `chat-instance.tsx` now background-refetches metadata when a slot returns to the foreground (`isActive` false→true), which restores the classic remount's freshness while preserving keep-alive's no-remount, keeps the cached value visible during revalidation (so `canonicalTitle` still drives the header), and makes the upstream spec pass **as written**. Pinned by `thread-title-sync.spec.ts` going green under keep-alive; the no-remount guarantee it rides on is pinned by `concurrent-chats.spec.ts` (both `[data-slot-key]` instances stay mounted across the navigation).

  **The third shape: an upstream spec asserts an upstream *policy* the fork deliberately does not have.** Seen on the 2026-09-14 sync: upstream PR #5397 added `settings-model-sync.spec.ts`, which expects a composer mode/effort edit to `PATCH /api/v1/auth/preferences`, because upstream syncs mode and reasoning effort account-wide and scopes only `model_name` per thread. This fork scopes **every** `THREAD_SCOPED_CONTEXT_KEYS` entry per conversation (§ per-conversation workflow), so a composer edit uploads nothing at all. Tell it apart from a genuine break by what the assertion is *named* for: the file's own claim — "preserves the account model" — holds more strongly here, because nothing is written. The spec is adapted to the fork's policy and its `patches` assertions pinned to exactly `[]`, never loosened to a subset, so a change that starts uploading per-conversation context still fails loudly. The same spec also needed its locators relaxed from exact label equality: the fork's model trigger renders a "Main agent" caption above the model name (a second, "Subagent", picker appears in Ultra mode), so `getByRole("button", { name: "Thread Model", exact: true })` matched nothing while the override was working perfectly.

  **Its cheaper sibling: the fork simply renders one more of something, and an upstream page-wide locator now matches twice.** Two arrived together on the 2026-09-06 sync, both from the workspace-level chat slots (§9). `agent-chat.spec.ts`'s IM-thread case visits `/workspace/chats/new` first, so that slot's composer stays mounted — hidden, but in the DOM, which strict mode still counts — and `getByPlaceholder(/how can i assist you/i)` resolves to two textareas. `thread-archive.spec.ts`'s mobile-header case measures the thread title's height, which more than one surface carries; its *custom agent* twin passed, because an agent route has no workspace slot — an asymmetry that reads as a real bug and is not. Tell this apart from the lifecycle case by the error: **`strict mode violation … resolved to 2 elements` is this one**, and the fix is scoping the locator to the surface the test names (`page.locator("header")`, `.filter({ visible: true })`), assertion untouched. A hang or timeout is the other one, and that fix belongs in the fork's code.

- [ ] **The backend suite passes with no `config.yaml` on disk.** `config.yaml` is gitignored: it exists on any machine that has run `make config` and on none of CI's runners. `make test` therefore tests a _different_ repository state locally than in CI, and the gap is silent in the direction that matters — a test that reaches for ambient config is green here and red there. Observed live on PR #71: seven tests that reached `AppConfig.from_file()` through an `app_config=None` default passed locally and failed CI with `FileNotFoundError: config.yaml file not found`. The rule for new tests is to **inject the config** (`AppConfig(sandbox=SandboxConfig(use="test"))`, or `app.dependency_overrides[get_config]` for a route) rather than letting a `None` default find the developer's file. Verify the way CI sees it:
  ...
  **The same gap, the other direction: `TEST_POSTGRES_URI`.** CI runs a Postgres
  service; a dev box usually has none, so ~175 tests `skipif` themselves away and a
  green local `make test` is **not** evidence CI is green. The 2026-09-12 sync proved
  it: `make test` reported `17713 passed, 175 skipped` while
  `test_persistence_forward_revision_compat.py::test_old_gateway_restarts_against_forward_postgres_revision`
  was red in CI. Anything touching migrations, the alembic graph, or bootstrap must be
  run with a real Postgres before pushing — the migration tree branches, so a
  downgrade that is single-headed upstream can leave this fork's
  `alembic_version` holding two rows:

  ```bash
  # throwaway cluster (CI uses postgres:17; 16 reproduces these). PGDATA must be
  # writable by the postgres user, so /var/tmp rather than an agent scratchpad.
  B=/usr/lib/postgresql/16/bin; D=/var/tmp/pgdata
  rm -rf $D && mkdir -p $D && chown postgres $D
  su postgres -c "$B/initdb -D $D -A trust -U postgres"
  su postgres -c "$B/pg_ctl -D $D -l /var/tmp/pg.log -o '-p 5432 -k /var/tmp' start"
  su postgres -c "$B/createdb -h /var/tmp -p 5432 -U postgres deerflow_test"
  cd backend && uv sync --all-packages --extra postgres
  TEST_POSTGRES_URI="postgresql://postgres@127.0.0.1:5432/deerflow_test?sslmode=disable" make test
  ```

- [ ] **`AGENTS.md` byte budgets — two different checks, and the second is the one that bites.** `backend/tests/test_agent_guidance_check.py` asserts every guidance file against its own **hard** budget (20/32/48 KiB); the standalone CI job (`scripts/check_agent_guidance.py`, run without `--strict-warnings`) warns at the soft ones (16/28/40 KiB) and errors at the same hard ones, so per-file the two agree. **The job is stricter about one thing: the effective chain.** A leaf inherits root + module + every guide down to it, and that sum has budgets of its own (80 KiB soft, **96 KiB hard**) — the hard one is a job _error_, not a warning, and so fails CI on its own. Most chains sit above the soft limit already, so only the hard one is a gate. The root file runs close to the line, so _documenting a feature can fail CI on its own_.
  ...
  **It has now happened twice.** The second time (2026-08-31, ComfyUI-on-by-default) was not a sync: a **six-line service-topology row** in the root file put `agents/middlewares/AGENTS.md` 564 bytes over the hard limit, failing both the job and `make test` on a module the change never touched. Note what did _not_ save it: the backend suite had been run — before the documentation edits. **A commit that touches any `AGENTS.md` now runs the checker through pre-commit** (`.pre-commit-config.yaml`, hook `agent-guidance`, repo-wide with `pass_filenames: false`, exiting 1 on errors and letting the soft-budget warnings through), so this is caught at the commit rather than in CI. That hook is pinned by `test_editing_guidance_is_gated_locally_before_ci_sees_it`; if it disappears, the trap is CI's problem again.

  **Three times now.** The 2026-09-05 sync put the same chain 635 bytes over and `subagents/AGENTS.md` 537 over. Both were paid the way this note predicted: root prose moved to pointers (the root is an ancestor of _every_ chain, so bytes out of it relieve all 29), and the leaf's 11.7 KB `acceptance_checks.py` paragraph moved beside the code as `subagents/ACCEPTANCE_CHECKS.md` — which is also how the 2026-09-12 sync absorbed upstream's expansion of it.

  **Current headroom is 528 bytes on the worst chain** (`agents/middlewares/`, 97776 of 98304) and 1190 on `subagents/AGENTS.md` (31578 of 32768). Upstream's own files account for most of the chain, so a sync of ordinary size lands here again and the fork is the only side that can give ground. Next candidates, in order: the root's repository-map comments, then `backend/AGENTS.md` (29180 bytes, the chain's largest fork-editable file). Check before pushing:
...
| **`web_search` / `web_fetch` backends**                                                                 | `cd backend && uv run pytest tests/test_web_search_dispatcher.py tests/test_detect_searxng.py tests/test_camoufox_stealth_options.py -q`. Three silent failures. (1) **The container still has to start.** `scripts/detect_searxng.py` picks the engine out of `backend:`/`fallback:`, not a provider module path, so a matcher that only greps `deerflow.community.searxng` reads the shipped Tavily-first config as "no SearXNG" and stops auto-starting the container on every launch path, with nothing red because `skip` is a legitimate outcome (`TestDispatcherConfigIsRecognized`; `test_a_neighbouring_tools_backend_key_does_not_leak_in` because web_fetch carries a `backend:` too, and `test_searxng_as_the_fallback_counts` because it is the *fallback* that needs it now). (2) **Empty is not an error.** The web_search fallback fires only on a raised primary, never on a search that succeeded with zero results (`test_an_empty_result_set_does_not_trigger_the_fallback`), and an unconfigured primary promotes the fallback instead of failing (`TestAnUnconfiguredPrimaryPromotesTheFallback::test_a_keyless_primary_runs_the_fallback_with_no_error` — a clone with no `TAVILY_API_KEY` must still search). (3) **Camoufox stealth is optional; web_fetch is not.** A launch that fails with the anti-detection options retries bare (`test_a_rejected_stealth_option_retries_bare`), while a *missing browser* still raises rather than being masked by that retry (`test_a_missing_browser_still_raises_and_is_not_retried`) — collapsing those two turns §31's actionable `make fetch-browser` message into a second identical failure. `test_the_jina_proxy_key_does_not_reach_the_browser` keeps web_fetch's jina `proxy:` out of the browser's options.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Large documents + scanned-PDF OCR** (§25)                                                             | `cd backend && uv run pytest tests/test_context_budget.py tests/test_context_aware_tool_output.py tests/test_document_extraction.py tests/test_document_chunking.py tests/test_document_ocr.py tests/test_document_analysis.py tests/test_analyze_document_tool.py` — the asserts that are silent when broken: `test_cloud_window_keeps_every_configured_value` + `test_unknown_budget_is_a_no_op` (a configured limit is a ceiling the window may lower, never a floor it raises); `test_an_explicit_disable_is_never_turned_back_on` (`0` means no limit, in both the sandbox and `tool_output` configs); `test_the_published_budget_does_not_leak_past_the_tool_call` (the ContextVar is scoped to one tool call, so a budget cannot outlive the model that produced it); `test_summarising_happens_after_transcription_not_during_it` + `test_the_instruction_forbids_summarising` (folding the two passes is cheaper and loses content); `test_anchors_do_not_count_towards_the_character_total` (else a 200-page blank scan reads as 4KB of content and never trips the sparse check); `test_a_fully_failed_document_is_not_cached` (caching an all-failed transcript makes the failure permanent); `test_notes_that_overflow_are_merged_in_rounds` (the reduce is hierarchical, or a long document just moves the overflow downstream); `test_the_ceiling_bounds_a_large_window` (`max_chunk_chars` reaches the chunker; a dead ceiling hands a 128K-window model ~55K tokens per map call); `test_a_capped_read_hands_back_the_part_to_resume_from` + `test_resuming_reads_the_next_parts_not_the_first_ones_again` (`max_chunks` returns a *prefix*, binding only on a small window, so truncation is invisible against a cloud model); `test_a_document_past_the_window_is_routed_to_analyze_document` (the size beside an upload is the *compressed* PDF, so without the extracted extent the agent reads linearly); `test_context_window_mirrors_num_ctx` in `test_sync_ollama_models.py` (without it the routing guard short-circuits on `None` for every local model). Wiring: `documents:` in `config.example.yaml` (`config_version` 54, incl. `large_document_chars`); `analyze_document` registered in `tools/tools.py` behind `documents.enabled`; `_context_clamped` at the three `sandbox/tools.py` truncation sites; `_document_quality` in `uploads_middleware.py` (scan warning + large-document routing). Manual: upload a scanned PDF with `uploads.auto_convert_documents: true` and confirm `<current_uploads>` flags it as image-based instead of showing an empty outline. Drift harness (§25a): `cd backend && uv run pytest tests/test_bench_drift_eval.py -q`, plus `make drift-eval`; silent when broken: `test_a_judge_that_passes_a_run_missing_the_fact_is_flagged` (without the planted facts a verdict is one model's opinion of another's).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **Full sandbox runs**                                                                                   | `skills/public/repo-runner/`; `sandbox.expose_ports` / `extra_capabilities` in `config.example.yaml`, honored by `LocalContainerBackend`. Container default (any Docker/Apple Container runtime, even non-interactively) + per-thread mode: `backend/tests/test_configure_script.py` + `test_docker_sandbox_mode_detection.py`; toggle: `test_sandbox_toggle.py`; forwarded `bash_command_timeout`: `test_local_sandbox_command_timeout.py` and `test_aio_sandbox.py -k TestPerCallTimeout`. **Silent when broken:** upstream's server-side `hard_timeout` superseded the fork's idle forwarding on 2026-09-22, and its `_effective_command_timeout` treated only `None` as unset — a `0` rode through as `hard_timeout=0` bounded at the 5s grace, aborting at once with the outcome unknown. Non-positive must mean *unset*. The remote path must **not** pass a per-call timeout either (the provider carries `default_command_timeout`; sending it twice is how one setting becomes two — `test_skill_request_scoped_secrets.py -k shared_timeout`); `expose_ports`: loopback bind, and no host publish at all when the API port is unpublished (upstream's restricted-network mode) — `test_aio_sandbox_local_backend.py -k expose_ports`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **Cost overview + aux counters** (§7)                                                                   | Shared `app/gateway/pricing.py` (console + thread endpoint import it); `GET /api/threads/{id}/token-usage` returns `total_cost`/`promo_total_cost`/`currency`/per-model `cost`/`aux`; store aggregation carries the input/output/cache-read split (`new_by_model_usage_entry`); `deerflow/runtime/aux_usage.py` records the four per-conversation sinks in `CHAT_AUX_CATEGORIES` — memory (`agents/memory/manager.py::_host_default_extraction_callback`), suggestions and prompt polish (`utils/oneshot_llm.py::run_oneshot_llm_with_usage`), and the goal check (`runtime/goal.py::evaluate_goal_completion`) — **write-through to the durable `deerflow/runtime/aux_usage_store.py`** (`<DeerFlow home>/aux_usage.sqlite3`, kill switch `DEER_FLOW_AUX_USAGE_DB=0`). **The quietest failure: a non-graph LLM call that takes a `thread_id` and is never given a category.** No `—`, no warning; the header simply prints less than was spent (`input_polish` and `goal` both shipped that way). Such a call needs a `CHAT_AUX_CATEGORIES` entry and a `record_aux_usage_metadata` call in the same change set — pinned by `test_aux_usage_wiring.py`. Frontend `token-usage-indicator.tsx` + `core/threads/token-usage.ts`. Pinned by `backend/tests/test_pricing.py`, `test_model_ids.py`, `test_aux_usage.py`, `test_aux_usage_wiring.py`, `test_thread_token_usage.py`, `test_thread_cost_robustness.py`, `test_input_polish_router.py`, `tests/blocking_io/test_aux_usage.py` + `frontend/tests/unit/core/threads/token-usage.test.ts`. **Run `test_thread_cost_robustness.py` for any change to pricing, the run stores, or the token-usage endpoint** — it holds three properties that are silent when broken: spend survives a restart (the aggregation counts every status that spent tokens, via `COUNTED_RUN_STATUSES` — narrowing it back to `success`/`error` makes a restart delete money from the header while the spend cap keeps charging), a mid-thread model switch is priced per turn at the model that ran it, and an edited message keeps its replaced turns in `total_cost` while excluding them from `steps[]` as `superseded_*`. **The aux registry's store is a local file, so its sync API blocks.** Re-pointing the suggestions route or the `token-usage` endpoint at `record_aux_usage` / `get_thread_aux_usage` instead of the `a*` wrappers fails the strict Blockbuster anchor — do not "fix" that by marking the anchor `allow_blocking_io`; restore the offload. Nor make the memory path async: it runs on the memory updater's loop-less debounce thread, which is why the durable store is its own SQLite file rather than the async runs engine. **Three things make this render `—`, none of which raises.** (1) Provider-id resolution (`pricing.py::_pricing_lookup_candidates`): buckets are keyed by the _provider-reported_ id, not the `config.yaml` one, so exact-only matching nulls every cost — pinned by `test_thread_token_usage.py::test_thread_token_usage_prices_provider_reported_model_ids`. (2) A bundled model with no `price:` block contributes nothing — pinned by `test_config_integrity.py::TestBundledModelPricing`. (3) The reported id can be _doubled_ (LangChain's `merge_dicts` concatenates equal `response_metadata` strings, so a provider sending several `finish_reason` chunks yields `deepseek/deepseek-v4-prodeepseek/deepseek-v4-pro`), which matches nothing and prices at zero — collapsed by `deerflow/model_ids.py::normalize_reported_model_name`, pinned by `test_model_ids.py` and `test_thread_token_usage.py::test_a_stream_duplicated_model_id_still_prices_and_is_named_once`. A bug report quoting a nonexistent model id is this one. **The second one has a delivery trap:** fixing `config.example.yaml` does **not** fix an existing install (`sync-api-key-models.py` skips active provider blocks; `merge_missing` cannot add a key inside a list entry), which is why `pricing.py` still derives a price from the `($in/out)` pair in `display_name` (pinned by `test_model_price_fields.py::TestBackwardCompatibility`) and why `TestBundledModelPricing::test_every_bundled_model_is_priced` requires a `price:` block on every bundled model. **Any change to the bundled model blocks must answer: does this reach a config that already exists?** `make doctor`'s `model pricing` check warns, naming the `—` symptom, when nothing configured can be priced. **A fourth failure is silent rather than visible:** an _expired_ promo — a promotion the provider has stopped still advertises a price nobody is getting. `TestBundledModelPricing::test_discounts_are_real_discounts_and_parse_their_expiry` pins that a bundled discount is below list and its `until:` parses (an unreadable one reads as _expired_); neither test knows whether the discount is still running, so re-verify live promos as step 6 of the [model audit](#auditing-the-model-list-settings--pricing). Cost is **per model everywhere**, the promo included: run buckets and the `aux` sinks are each priced at their own model's rate (`test_promo_total_is_model_aware_across_lead_subagent_and_aux`). Manual: run an Ultra-mode turn (so a subagent model is involved) and confirm the header shows a **green** dollar amount; on a discounted model the dropdown shows the green promo total beside the red standard one; on `—` the dropdown names the unpriced model. |
...
| **Model dropdown sorting/grouping** (§8)                                                                | `cd frontend && pnpm test sorting` exercises the parse/sort/group logic (`frontend/tests/unit/core/models/sorting.test.ts`). Wiring: `core/models/sorting.ts` (`parseModelPrice` promo-aware, `parseModelProvider`, `sortModels`, `groupModelsByProvider`, `demoteLast`); preference `modelPicker` in `core/settings/local.ts`; shared UI `components/workspace/model-picker-controls.tsx` (`ModelPickerControls` + `ModelPickerList` + `ModelDisplayName`) used by the lead + subagent pickers in `input-box.tsx` and the sidecar picker in `sidecar/sidecar-panel.tsx`; **`components/workspace/model-select.tsx` (`ModelSelect`) is the same picker packaged for every other site** — `democracy-setup.tsx`, `settings/suggestions-settings-page.tsx`, `settings/subagent-settings-page.tsx`, `agents/agent-generator.tsx`, `agents/agent-settings-dialog.tsx` — pinned by `frontend/tests/unit/components/workspace/model-select.dom.test.tsx` and, structurally, `model-picker-sites.test.ts`. **Tests locate a picker by its own `data-slot="model-select"`, never by `getByRole("combobox")`** — that role belongs to whichever primitive is underneath, so it vanished when the primitive was swapped and took `suggestions-settings.spec.ts` down with a 30s timeout while the product itself was fine. `model-picker-sites.test.ts` fails in milliseconds on both halves of that (a dropped `data-slot`, or any spec driving a model picker by the combobox role); i18n keys in `core/i18n/locales/{en-US,zh-CN}.ts`. **Row layout** (provider first, price pinned to the right edge, weights + window on the second line) is `modelRowParts` + `ModelPickerRow`, pinned by `sorting.test.ts` (`parseModelProviderLabel`, `formatModelSize`, `formatContextWindow`, `modelRowParts` — including the legacy name whose price group must be removed _whole_ rather than leaving `GLM-5.2 ( → )`), by `model-select.dom.test.tsx` (the provider leads the row, the price span carries `ml-auto`, a local model's `id · GiB · K ctx` line, and grouped mode dropping the per-row provider), and structurally by `model-picker-sites.test.ts`, which fails any component driving `ModelPickerList` without `ModelPickerRow`. **Upstream now ships a rival picker.** Its 2026-09-22 `model-picker-content.tsx` (favorites, no search) replaced this pair upstream and deleted `ai-elements/model-selector.tsx`; the fork keeps its own picker and keeps that component alive. Repointing `input-box.tsx` or `sidecar-panel.tsx` at upstream's compiles, type-checks and renders — it just silently trades search, sorting, grouping and prices for favorites. `model-selector-overflow.test.ts` fails both directions (the fork's three pieces missing, or upstream's components wired in). Upstream's `tests/e2e/model-favorites.spec.ts` is **deleted on each sync that brings it back**: it locates the trigger by the bare model name, which this fork's trigger prefixes with "Main agent"/"Subagent", so all four tests fail before any favorites assertion. Delete it again, or integrate favorites into `ModelPickerList`/`ModelPickerRow` — the store arrived intact in `core/models/{favorites-store,favorites,use-model-favorites}.ts` — and rewrite the spec against this fork's DOM. Manual: open the dropdown → Sort (Default/Name/Price), direction toggle and Group-by-provider appear and reorder the list; prices render green against the right edge and the discounted entry shows its red list price beside the green promo. A whole name turning green, or an uncoloured price, means `modelNameSegments` drifted from `price:`. Then **close** the dropdown on a discounted model at a narrow width: clipping means the `w-full` on the three `ModelSelectorName` triggers was dropped (§8) — CSS with no unit test, so it needs the eye. Then check a non-chat screen (Democracy setup, Settings → Suggestions/Subagents, the agent generator, a custom agent's dialog): a plain grey list means a site was added without `ModelSelect`. |
| **Live chat slots** (§9)                                                                                 | `cd frontend && pnpm test live-chat-slots` covers the pure model (`frontend/tests/unit/core/threads/live-chat-slots.test.ts`: one slot per thread, the ceiling **declining** rather than evicting, a stable key surviving a new→real promotion) and the provider (`live-chat-slots-busy.dom.test.tsx`). Wiring: the live chat is `components/workspace/chats/chat-instance.tsx` (**fully controlled**, own provider stack via `chat-providers.tsx` with a per-instance `storageScope`); `keep-alive-chat-viewport.tsx` is mounted in `workspace-content.tsx` **above** the route inside `LiveChatSlotsProvider` and renders one instance per slot (only the active shown, the rest `display:none`); `[thread_id]/page.tsx` is a thin registrar in app builds and the classic inline `<ChatInstance>` in static-demo; state in `core/threads/live-chat-slots-context.tsx`; `ChatBox` panel ids keyed by thread id (not pathname). **The user-managed tab strip this replaced is gone on purpose** (folders do that job, §32) — a sync that reintroduces `chat-tabs*`, `/api/settings/chat-tabs` or a `chatTabs.*` i18n block is re-adding a removed feature, not restoring one. **If upstream restructures `[thread_id]/page.tsx`,** re-extract its body onto `chat-instance.tsx` and keep the registrar/classic split; watch for a barrel (`components/workspace/chats/index.ts`) import of the client viewport into the server `workspace-content.tsx` (import the file directly to keep the `"use client"` boundary). **Upstream may also extract the same body into a _new_ file** — #4765 added `chats/chat-page.tsx`, a slot-less duplicate missing the fork's cost header. An _add/add_ reports no conflict, so the duplicate silently becomes where upstream's future chat work lands; it is **deliberately deleted in this fork**. Port any delta into `chat-instance.tsx` and delete it again. |
| **A conversation's cost survives an edit** (§7)                                                          | `cd backend && uv run pytest tests/test_thread_cost_robustness.py -k EditingIntoAVersionThread`. An edit branches into a **new thread id**, so aggregating the open thread alone reported a total lower than the money spent *and* a chart missing the turns still on screen. `app/gateway/edit_version_family.py` resolves the family from thread metadata (`deerflow_edit_version*`, written by `frontend/src/core/threads/edit-versions.ts` — renaming a key on one side silently returns every conversation to single-thread costing), and the endpoint sums every member while charting only the open lineage. The asserts that are silent when broken: the version thread reports the **whole family's** total, its chart includes the inherited turn that ran in the *parent* thread, `sum(steps) + superseded_cost == total_cost` still holds across the branch, and reading the original wording through the `‹ 1/2 ›` switcher reports the **same** total. Degradation is deliberate and pinned too — no thread store, or a looping parent link, costs the open thread rather than failing the request. `is_single_thread` keeps an unedited conversation on the old path byte-for-byte. |
| **A chat remembers its own model** (§36)                                                                 | `cd backend && uv run pytest tests/test_thread_workflow_metadata.py` + `cd frontend && pnpm test thread-workflow thread-context-account-boundary && pnpm test:e2e thread-workflow-memory`. The selection rides on the thread (`deerflow_workflow`), so another device shows **its** model and mode. Silent failures: a loose value guard hands the `updated_at` exemption to arbitrary metadata (the sidebar stops re-sorting); a promotion without `copyThreadContextOverride` + `recordOnPromotion` (`onStart`) forgets the model on turn one; without `workflowChanged` the composer PATCHes every render; and a `resolveThreadContext` that pins its fallback as the chat's override (passwordless has no account sync) makes the chat refuse its record, then PATCH the fallback over it. |
| **Update dependencies button** (§37)                                                                     | `cd backend && uv run pytest tests/test_dependency_update.py`. `POST`/`GET /api/settings/update-dependencies` drive `scripts/update_camoufox_searxng.py` on demand from **Settings → Maintenance**. Four properties, each a way it could be worse than absent: single-flight (two concurrent `docker pull`/`camoufox fetch` runs fight over the same files), the POST **returns without doing the work** (holding it open outlives every reverse-proxy read timeout), one component failing or hanging never sinks the other, and the admin gate — this runs commands on the host and takes **no** caller input. |
| **Concurrent chats** (§21)                                                                              | `cd backend && uv run pytest tests/test_concurrent_thread_runs.py` pins that two threads stream at once (the cross-thread test rendezvouses, so a process-global run lock times out instead of passing) and that two runs in one thread still serialize. `cd frontend && pnpm test run-disconnect live-chat-slots` covers the frontend halves: every `thread.submit` sends `onDisconnect: "continue"` (the Gateway's default is `"cancel"`, and the SDK's own is derived from the `streamResumable` flag `sanitizeRunStreamOptions` strips — so it must stay explicit); `syncRoute` **retains** the slot it is leaving while that slot is streaming instead of dropping it (not retained when the slot has no real thread id yet; a full set declines rather than evicting); returning to a background chat **adopts** its instance rather than mounting a second one on the same thread; and the slot is **released** once its run leaves `busyKeys` — via an effect, so it never pre-empts the stream's own `onFinish` notification. `pnpm test:e2e concurrent-chats` is the whole user story in one run: prompt chat A with its SSE response withheld, navigate to chat B, assert both `[data-slot-key]` instances are mounted, send in B and get its answer while A is still unanswered, then release A, watch the background slot drop back to one, and reopen A to find its answer. Ollama half: `cd backend && uv run pytest tests/test_ollama_lifecycle.py -k "Parallel or concurrency"` — `ollama.num_parallel` parsing, CLI-over-config precedence, N slots dividing the sized `num_ctx`, slots counted in the contention warning, and the `make doctor` line.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| **Alembic merge points** (see _[Upstream sync](#upstream-sync)_)                                        | `cd backend && uv run pytest tests/test_migration_merge_points.py`. Two heads are loud (bootstrap refuses to start); the row is for the *silent* wrong fix. Re-parenting one branch onto the other's tip also yields one head and passes everything else, while a database stamped at the re-parented revision reads as at head with the other branch's DDL never applied. Asserts one head, every merged tip still an ancestor of it, and each branch root still naming its original parent — so fix a collision with another merge revision, never a re-parent. `0027_merge_preferences_mcp_tasks` is the third instance (2026-09-22), which also caught **upstream re-parenting its own** `0023_user_preferences` onto `0023_run_change_seq` (#5405) — recorded, not reverted: upstream owns that branch and ships `0025_repair_run_change_seq` for the databases it stranded. Upstream migration tests assuming a linear chain must target the fork's merge points, or they build a `runs` table with no `pricing_snapshot` and the ORM fails on it. |
| **Automated upstream sync** (see _[Upstream sync](#upstream-sync)_)                                     | `cd backend && uv run pytest tests/test_upstream_sync.py` covers parsing this checklist out of FORK.md, the PR body for clean and conflicted merges, gate rendering (pass / fail / **skip**, which must stay distinguishable — a skipped gate reading as green is how a conflicted sync looks mergeable), the 65536-character GitHub body limit (the body renders past it today, so **pay for a new row by shortening one** — see [CHANGE_CYCLE.md](CHANGE_CYCLE.md) step 4), and workflow invariants. Wiring: `scripts/upstream_sync.py`; `.github/workflows/upstream-sync.yml` (weekly + `workflow_dispatch`). **The body is generated from this section, never copied**, so the two parsers are load-bearing: gates come from the `- [ ]` lines and features from the table rows, and renaming this heading or restructuring the table silently empties the PR body (`test_the_real_fork_md_parses` guards it). Workflow invariants pinned by tests: `git merge` never `git rebase`, no `--force` in any form, and the PR step under `if: always()` so a conflicted merge still surfaces.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **Gaslight mode — edit into a hidden version** (§18)                                                    | `cd backend && uv run pytest tests/test_threads_router.py -k answer` covers the answer half end to end: the branch rewrites only the edited assistant message, the run-event seed carries the replacement (the feed reads events, not the checkpoint), a branch without the pair is byte-unchanged, and every half-specified or out-of-turn rewrite is refused. `cd frontend && pnpm test edit-version-answer && pnpm test edit-versions && pnpm test pending-edit-send && pnpm test "core/messages/utils"` covers the version model (group keying on the base message id, lineage resolution, descendants inheriting an ancestor's position, the malformed-entry guards), the session-storage hand-off (read consumes it, so an edit never replays twice), and the per-turn edit anchors. `pnpm test:e2e edit-message-versions` drives the whole flow: edit a middle turn, land on the version with the earlier history and without the replaced answer, **the inherited answer still rendered above the replayed turn**, one sidebar entry pointing at the version, `2/2` on the edited message, switch back to `1/2`. That order assert is load-bearing and depends on a fixture contract: the mocked feed must stamp one `run_id` per **turn**, as the gateway's branch seed does (`branch-seed-{thread}-{n}`). A per-thread id makes the inherited answer look like this turn's own output, `restoreLocalTurnMessageOrder` drags it below the new human, and the switcher — anchored on that answer — silently disappears. Wiring: `core/threads/edit-versions.ts` (model + metadata keys), `pending-edit-send.ts`, `useCreateEditVersion` / `useSetActiveEditVersion` in `hooks.ts`, `chats/use-edit-versions.ts`, `messages/message-version-switcher.tsx`, `MessageList`'s `onEditMessage` / `editVersionSwitchers`, the `deerflow_edit_version` filter in `thread-search-query.ts`, and the active-version hop in `pathOfThread`. **Five things are silent when broken:** (0a) an **answer** edit must not park a pending send — the branch already carries the rewritten answer, so parking one replays the assistant's words as the user's next message; (0b) answer groups must stay namespaced (`answer:<id>`) — an answer edit on turn _k_ and a prompt edit on turn _k+1_ branch from the same message, so a shared key renders both version sets on both; (1) groups stay keyed on the **base message id** — a turn-index key merges lineages that only share an ordinal; (2) `pathOfThread` keeps honouring `deerflow_edit_active_version`, or the sidebar entry reopens version 1 forever and the edit reads as lost; (3) `takePendingEditSend` keeps _removing_ on read — a non-consuming read replays the edited turn on every remount. If upstream restores a Branch button on the assistant action row, decide deliberately: this fork removed it on purpose. Manual: edit the first message of a chat (the no-branch path), confirm the switcher appears, then reload from the sidebar and confirm you land back on the edited version.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Local image generation** (§23)                                                                        | Its tests all run under §26 (superset; re-check with `--collect-only` if that `-k` narrows). Properties §26 does not state: the template contract (typed parameters only, an unbound parameter refused rather than ignored, no path traversal in a template name, and a patched graph that cannot leak back into the cached template), validation against `/object_info` naming the node that moved _before_ anything is submitted, checkpoint resolution from the loader node's own enum (request → config → first installed) and the "not installed" message that lists what is, the SSRF guard being **used** with its documented `allow_private_addresses` opt-out rather than bypassed, the submit → poll → fetch loop against a mock transport including its wall-clock timeout, and the detector reusing an already-running ComfyUI instead of starting a second one. **Silent when broken:** the saved `<name>.workflow.json` must equal the graph that actually ran and must not carry binding metadata (otherwise "reproducible by hand" quietly stops being true); the reported seed must be the seed that reached the graph (otherwise every iteration is a fresh random draw and critique is noise); the active tool entries and the launch-time provisioning are **one decision** and must move together (owned by §26, which pins it); and the compose port must stay loopback-bound — the ComfyUI API has no authentication and can read and write host files. Manual: `make dev` on a GPU machine with no ComfyUI running, confirm the bundled container comes up on its own, ask for a picture, confirm the PNG opens in the artifact panel and that the sidecar workflow file loads in ComfyUI and reproduces it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **GPU residency arbiter** (§23)                                                                         | `cd backend && uv run pytest tests/test_gpu_arbiter.py` (doctor half runs under §26). Covers the computed policy (a small card resolves to `exclusive`, a bigger one to `shared` **on its own**, an unknown estimate or unknown budget falls back to exclusive and names why), eviction on acquire _and_ self-eviction on release, residency re-read from the services on every acquire, the `nvidia-smi` tiebreak that evicts when VRAM is held while no tenant claims it, serialization of two concurrent generations, the honest timeout instead of an unbounded queue, and the doctor row for VRAM held while idle. **Silent when broken:** Ollama's unload must stay a **per-request** `keep_alive: 0` — writing it globally reintroduces the subagent cold starts `ollama.keep_alive` exists to prevent; a cloud tenant must remain not-resident rather than gaining a branch of its own; and dropping the release-time self-eviction leaves the diffusion weights on the card, which does not fail, it just makes the next chat turn several times slower. Manual: with a local lead configured, generate twice in a row and confirm `nvidia-smi` shows an empty card between generations.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **Through each lab's own API, with a fuller lineup.** That same flagship
  ...
  `ANTHROPIC_API_KEY` brings Fable 5.1, Opus 5.5, Opus 5, Sonnet 5, Sonnet 4.6 and
...
- **Anthropic (direct key) — the Claude lineup, by family generation:**
  - **Opus** and **Sonnet** each keep their **previous generation _and_ the current one** — i.e. Opus 5 + Opus 5.5, Sonnet 4.6 + Sonnet 5. These are the tiers people pin threads to, so the outgoing model stays alongside its successor rather than being dropped the day the successor ships.
  - **Haiku** and **Fable** keep **only the latest** — Haiku 4.5, Fable 5.1. The small/cheap tier and the top "most capable" tier each only need their current model; nobody pins an old Haiku or an old Fable.
  - When a new generation ships (say Opus 6), the rule rolls forward mechanically: the new model joins, the now-second-newest Opus stays as the previous-generation pin, and the third-oldest drops. Opus 5.5 was the first roll (2026-09-26): Opus 4.8 left the bundle.
...
audit itself still detects drift. **That fixture is regenerated by hand and
belongs to the roster**, so a roll-forward that skips it leaves routed models
out of the catalog, where they read as `retired` — findings for models nothing
retired, indistinguishable from the one slug it deletes on purpose. Merely
producing findings proves nothing, which is why `TestStaleFixture` now also
pins the *shape* its `_comment` promises: exactly one absent slug, and exactly
four findings, one per drift kind. Found live on 2026-09-15, two roster rolls
after both halves of the paired-lab rule went missing.
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `ANTHROPIC_API_KEY`  | Claude **Fable 5.1**, **Opus 5.5**, **Opus 5**, **Sonnet 5**, **Sonnet 4.6**, **Haiku 4.5**                                                                                                                                                   | direct Anthropic API (`langchain_anthropic:ChatAnthropic`)                   |
| `OPENROUTER_API_KEY` | Claude **Fable 5.1**, **Opus 5.5**, **Grok 4.6**, **GPT-6 Astra**, **GPT-5.6 Sol**, **GPT-5.3 Codex**, **Gemini 3.8 Flash**, **Llama 4 Maverick**, **MiniMax M3**, **Qwen3.8 Max**, **Kimi K3**, **Mistral Large 3**, **DeepSeek V4 Pro**, **GLM-5.3**, **Nemotron 3 Ultra** | OpenRouter (`langchain_openai:ChatOpenAI` + `base_url`) |
...
| `GEMINI_API_KEY`     | **Gemini 3.8 Flash**, **3.5 Flash-Lite**, **3.1 Pro**                                                                                                                                                                                       | native Gemini SDK (`langchain_google_genai:ChatGoogleGenerativeAI`)          |
...
### 9. Live chat slots (a chat you leave keeps answering)

Upstream shows one live chat at a time: the sidebar lists your conversations, and clicking one navigates the single content pane — switching away tears the previous chat down (its stream, scroll position, and artifact panel are gone until you come back and reload). This fork lifts the live chat **above** the route so the chat you are looking at, plus any chat still answering after you left it, stay mounted.

**This replaced a user-managed tab strip** (drag conversations onto a strip to pin them; persisted server-side per user). The strip was removed once [sidebar folders](#32-folders-in-the-sidebar-chat-tree) shipped: filing a conversation is the durable way to keep it to hand, and a pinned strip was a second, competing place to do the same thing — with its own drop-target rules, its own persistence, and its own reconciliation bugs. What is left is the half that was never a preference: a chat cannot be unmounted while it is still answering, because unmounting it is what made leaving a chat feel like cancelling it.

- **What "keep-alive" means here.** The live chat was lifted out of the route into a persistent, workspace-level viewport (`keep-alive-chat-viewport.tsx`) mounted **above** the Next route, so navigating between chats never unmounts them. It renders one chat instance for the current chat plus one per background chat, and only the active one is shown — `display:none` on the rest keeps React state, DOM scroll position, and the SDK stream alive. Background chats survive navigating to other workspace pages too (the whole viewport is just hidden). The new→real rewrite uses `history.replaceState`, not the Next router, so nothing remounts.
- **A background slot is scoped to one run, and nothing is persisted.** Leaving a chat that is streaming retains its slot (reusing the slot key, so the instance is not torn down); the provider releases it once that slot leaves `busyKeys`. The release runs in an effect, i.e. after the stream's own `onFinish`, so it can never pre-empt the "conversation finished" notification. There is no stored set to reconcile, no `localStorage` cache, and no `/api/settings/chat-tabs` — an install written before the removal keeps an inert `chat_tabs` key in its `ui_state.json`, which is simply never read.
- **Two deliberate limits.** A brand-new chat whose thread the backend has not created yet is **not** retained (a background slot is addressed by thread id, which it does not have), and the background set is capped at `MAX_BACKGROUND_CHAT_SLOTS` (8) — each one holds a live instance, so the ceiling is a resource guard. In both cases the run still survives server-side (`onDisconnect: "continue"`) and is rejoined on the next visit.
- **The single chat is a reusable, controlled component.** `[thread_id]/page.tsx`'s body became `chat-instance.tsx` — **fully controlled** (the owner owns `threadId`/`isNewThread`; the instance reports its new→real promotion up) and wrapping its own provider stack with a per-instance `storageScope` so several artifacts panels never collide on one pathname. In app builds the route page is a thin **registrar** that reports the route and renders nothing; in **static-demo** builds the feature is off and the page renders the classic inline chat (the demo pre-renders these pages and enforces route asset budgets). Custom-agent chats keep the classic single-chat rendering.
- **Metadata revalidates on foreground, because nothing remounts to do it.** The classic single pane refetched a thread's metadata (`useThreadMetadata` — title, goal) every time you navigated back to it, because the page remounted. Keeping the instance mounted removed that free refresh: a kept-alive thread's title/goal would freeze at first-load and only a full reload picked up a change made elsewhere (a rename in another browser, another client). `chat-instance.tsx` restores it explicitly — a background refetch fired when a slot returns to the foreground (`isActive` transitions false→true, skipping the first mount, which the query already fetches). It is a background revalidation, so the cached value stays on screen while it runs (the header's `canonicalTitle` never blanks). This is also what makes the upstream title-sync race spec pass — see the post-sync checklist's e2e-collision note.

**Where it's wired.**

| Piece                                                     | Location                                                                                                                          |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Pure slot model (retain/release/promote)                  | `frontend/src/core/threads/live-chat-slots.ts`                                                                                    |
| Slot state + route/busy coordination                      | `frontend/src/core/threads/live-chat-slots-context.tsx` (`LiveChatSlotsProvider` / `useLiveChatSlots`)                             |
| Persistent viewport                                       | `frontend/src/components/workspace/chats/keep-alive-chat-viewport.tsx`                                                            |
...
| Shell mount                                               | `frontend/src/app/workspace/workspace-content.tsx`                                                                                |
| Sidebar drag payload (shared with folders)                | `frontend/src/core/threads/dnd.ts` (`CHAT_DND_THREAD_MIME`)                                                                        |

Pinned by `frontend/tests/unit/core/threads/live-chat-slots.test.ts` (the pure model: one slot per thread, the ceiling declining rather than evicting, and the stable key surviving a new→real promotion) and `frontend/tests/unit/core/threads/live-chat-slots-busy.dom.test.tsx` (the provider: a busy chat retained on the way out, an idle one not, a not-yet-created one not, a full set declining, returning to a background chat adopting its instance rather than mounting a second, and the slot released once its run lands).

**Verify it works.**

```bash
cd frontend && pnpm test live-chat-slots     # pure model + the provider's route/busy coordination
cd frontend && pnpm test:e2e concurrent-chats  # the whole user story, end to end
```

Then end-to-end (`make dev`): ask one chat for something slow, switch to another chat while it is still answering, confirm both instances are in the DOM (`[data-slot-key]` × 2) and the second chat answers normally; when the first answer lands you get its notification and the background instance is dropped again. Full frontend gate: `pnpm format && pnpm check && pnpm test`.
...
**as a unit**: memory, threads, folders, runtime settings, uploads, the
...
per user in the same `ui_state.json` bag as the sidebar folders, deduped by
...
was scoped alongside this is _not_ done. The artifact panel is still
desktop-shaped on a narrow viewport. The PWA shell,
...
2. The new thread is stamped `deerflow_edit_version: true` and is filtered out of
   ...
   in the sidebar or the chats page.
...
### 25a. Measuring the failure where the run succeeds and the goal is gone

§25 made a long document *readable*. This measures whether reading it costs the
model the question it was asked.

The failure has no error attached to it. The document fills the window, the
instruction ages out or is compacted away, and the model settles into the task
its surviving context suggests — in practice, almost always "summarise this
document" in place of the specific lookup it was given. The output is fluent,
on-topic, and wrong about its own purpose, which is exactly why it survives
review. No assertion in the suite fires, because nothing failed.

**Why a judge model alone is not the measurement.** The obvious design is to ask
a second model "did this drift?". It works less well than it looks: a judge
grading a confident, well-written answer agrees with it more often than it
should, and a harness built on that produces a number that drifts in the same
direction as the thing it is measuring. So the corpus is **generated**, with
facts planted at known pages, and the answer is checked mechanically against
them as well. When the judge passes a run that missed its planted fact, the
harness reports that as a **judge** failure. That disagreement is the signal
worth having: without it the whole exercise is one model's opinion of another's.

Three decisions that are load-bearing:

- **Placement is the measurement.** The facts sit early, at two-thirds, and near
  the end. A read that stops at `max_chunks` recovers the early one and misses
  the late one, so a run reporting success on the early fact alone is a partial
  read presenting as a complete one. The default question targets the last fact,
  on the page a capped prefix read never reaches.
- **Correctness and drift are graded separately.** A wrong answer read the right
  pages and got the fact wrong — a model-quality problem. A drifted answer
  stopped pursuing the question — a context problem, and the only one of the two
  a bigger window or different chunking would fix. Scoring them together hides
  the thing being measured.
- **A partial match is not a hit.** Recall matches the distinctive tokens of the
  expected value, because a correct answer rephrases; but *all* of them must
  appear. Counting "the date is 2031-04-18" as answering "what was it before it
  moved" is precisely the accounting error that makes a truncated read look
  complete.

Two modes share one rubric so a criteria change is a diff in one file:
`pipeline` calls `analyze_document_text` directly and needs nothing running;
`agent` drives the whole loop over Gateway HTTP, which is the only place the
document actually competes with the instruction for one window. Both import
production code — a result is only worth having because it is a statement about
the code that ships. `--no-follow-resumption` reproduces the pre-fix silent
prefix read, which is how the cost of that bug is a number on the same document
and model rather than an assertion.

The corpus is synthetic, self-identifying and never downloaded, and the offline
tests need no credentials. Depth lives beside the code in
...
- **The static list restores too, from the DOM.** Below the threshold the rows
  are real DOM with real heights, so the restore reads the anchor row itself,
  never a virtualizer estimate. It used to be left to CSS scroll anchoring,
  which does nothing for a scroller at offset 0 — exactly where the load-more
  sentinel fires — so each older page parked the reader at its top and the
  turns in between were skipped. Anchors are kept in viewport terms on both
  paths, which is what lets a thread cross the threshold — as a loaded page
  routinely makes it — without losing the reader.
- **The anchor is never the list's first group while another is on screen.**
  Pages are cut by row count and an agent turn is many rows, so the newest page
  nearly always opens mid-turn; the older page completes that turn by merging
  into group 0, which re-keys it. Anchored there, the restore found nothing and
  did nothing. `pickPrependAnchor` passes over index 0.
- **A virtualized restore is re-seated on the measured row.** Its offset is
  built from estimates for every row the page brought in, and virtual-core
  judges whether to compensate a re-measured row from its own scroll offset,
  which has not caught up with the restore — so it does not. `settleAnchorOnRow`
  corrects against the rendered row for a few frames; without it the reader
  lands a few hundred pixels off.
...
Its two "completes the top turn" siblings are the other kind: deterministic,
and red on the old code. They feed rows shaped like agent turns — a question,
tool calls and results, an answer — with the page boundary inside a turn, which
one-row-per-group fixtures cannot express, and each goes red on its own line:
the static restore, the first-group skip, and the re-seat.
...
The side list was one flat, recency-ordered column: pinning kept a handful of conversations at the top, and everything else scrolled. This fork adds **folders**, and they behave the way a file manager's do — a conversation dropped into a folder is *inside* it and **no longer in the list outside it**, the folder collapses to a single row you open with the arrow beside its name, and **folders nest inside folders**. Create one with the **New folder** button beside the "Recent chats" heading, file conversations by dragging them onto a folder (or via **Move to folder** in a chat's row menu), nest a folder by dragging it onto another (or via **New subfolder** / **Move folder to** in its own menu), and rename or delete a folder from that same **⋯** menu — the same affordance the conversation rows already carry.
...
- **The tree is a `parentId` on each folder, and the store is what guarantees it is a *forest*.** Nesting is one optional field — `parentId`, absent for a top-level folder — rather than a nested document, because the flat list is what every other operation already works on: a rename, a delete and a reorder all stay one pass over one array, and a flat list still round-trips byte-for-byte through the feature. What a flat list cannot enforce for itself is the shape, so both `normalizeChatFolders` and its Python mirror `normalize_chat_folders` repair the parent links on **every** read and write: a parent that names no folder, a chain that loops, and anything deeper than `MAX_FOLDER_DEPTH` (5) are all fixed. **The repair is always "move it to the top level", never "drop it"** — that is the same rule as the thread fallback above, applied one level up. A dropped folder takes the conversations filed inside it out of their folder with no way back; a folder that surfaces at the top is visible, named, and one drag from where it belongs. A cycle is the case worth naming: it is unreachable from every root, so a renderer that walks down from the roots loses the whole branch **and every chat in it** with no error anywhere. Repairs are applied one folder at a time with the depths re-derived in between, because orphaning the *shallowest* offender of an over-deep chain brings everything below it back into range in one move.
- **The rules that keep a drag from destroying the tree.** `canMoveFolderUnder` refuses three moves, and all three are silent disasters if allowed: a folder into itself, a folder into its own descendant (both detach the subtree from every root — the folders stop rendering and their chats fall back to the root list, with nothing saying why), and a move that would push the deepest descendant past the depth limit. The refusals are surfaced *before* the gesture wherever possible rather than after it: a folder already at the limit does not accept a folder drag at all (its `dragover` never calls `preventDefault`), **New subfolder** is disabled on it, and an invalid target in **Move folder to ▸** is greyed rather than hidden — a target that silently disappears reads as a bug, a greyed one reads as a rule.
- **A nested row lives inside its parent's drop zone, so the innermost target has to claim the drop.** A subfolder renders inside the parent's expanded child area — which is what makes collapsing a parent collapse the whole branch for free, since the branch lives in the element that stops rendering. The cost is that a drop on the child bubbles to the parent's handler, which files the chat one level too high; that looks exactly like a drag that missed. `ChatFolderRow` therefore calls `stopPropagation` on `dragover`, `drop` **and** `dragstart`. The count on a folder row is likewise the **whole subtree**, not the direct members: a collapsed parent reading "0" while holding twenty conversations is the same class of lie.
- **Deleting a folder deletes its subfolders, and never a conversation.** The subtree goes, the way a file manager works — leaving a subfolder behind would make it an orphan that only reappears because the normalizer promotes it, which is a repair, not a behaviour anyone asked for. Every loaded chat filed **anywhere in the subtree** has its `deerflow_folder` cleared, not just the direct members, and anything not loaded is caught by the unknown-folder fallback.
- **The registry and the membership are stored in two different places, on purpose.** The folder list — id, name, display order — is per-user server state in `{base_dir}/users/{user_id}/ui_state.json`, exposed as `GET`/`PUT /api/settings/chat-folders`, ungated for the same reason: it is per-user UI state, not a server-wide setting. *Which* conversation is in *which* folder rides on that conversation's own thread metadata (`deerflow_folder`). Renaming a folder is therefore one small write instead of one write per conversation inside it, and a conversation carries its filing with it wherever the thread record goes. Do not "simplify" this by storing member ids in the registry: a rename is the cheap operation, and a membership list would need reconciling against every delete, branch and edit-version the thread list already performs.
...
- **One drag, one payload.** A sidebar chat row advertises its thread id under `application/x-deerflow-thread-id` (`core/threads/dnd.ts`), which is what files the chat into a folder. Rows are **always** draggable. A folder row is a drag source too, advertising its own id under a **separate** `application/x-deerflow-folder-id` MIME — separate because a target decides what it accepts by reading the type list, and that is what lets a folder accept a chat while refusing a folder it cannot take. Drop targets are explicit and disjoint: a folder header (a chat, or a folder to nest), a folder's expanded child area (a chat), and the root list container (a chat out of its folder, or a folder back to the top level — without it a nested folder could only ever be promoted through its own menu). Nothing else accepts a drop, so there is no ambiguous middle ground where a drag silently does nothing or does the wrong thing.
- **Drag-and-drop is the fast path, never the only path.** Native HTML5 DnD is unusable by keyboard and by screen readers, so every folder operation also exists in a menu: **Move to folder ▸** in a chat's row menu (with the folder tree indented one step per level, a **No folder** entry, and a **New folder** shortcut), and **New subfolder**, **Move folder to ▸** (with a **Top level** entry), rename and delete in the folder's own menu. The group header's control is a real button, it sits **beside the heading rather than at the sidebar's right edge**, and it is **labelled "New folder" rather than drawn as a bare `+`**. Two passes were needed to learn the same lesson twice: an unlabelled icon a sidebar-width from the words it belongs to is in the DOM without being findable, and moved next to those words it is *still* only findable by a reader who already knows folders exist — a 16px glyph against a 12px muted heading reads as part of the title. The words are what turn it into an invitation, so they are rendered, not hidden in an `sr-only` span that satisfies a text assertion while showing the reader nothing. The header is also rendered **whether or not anything is under it** — a workspace with no conversations is exactly where the first folder gets made, and a group that returns nothing until a chat exists takes the only control that creates one with it.
- **Expanded/collapsed is per browser; the folders themselves are not.** Which folders are open lives in `localStorage` under one key — the same tree can reasonably be open on a desktop and collapsed on a laptop, and it is the one piece of this feature nobody would miss after a data clear. A folder the user *just created* is opened automatically, so the drop they are about to make has a visible target — and so is the folder it was created **inside**, or a new subfolder would appear in a collapsed row and read as nothing having happened. Re-parenting a folder opens its new parent for the same reason. Everything else — names, order, membership — is server state that follows the user across browsers and devices.
...
| Piece                                                                     | Location                                                                                                                                                                       |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Pure model (create/rename/remove/**move**, tree repair, the grouping rules) | `frontend/src/core/threads/chat-folders.ts`                                                                                                                                    |
...
Pinned by `frontend/tests/unit/core/threads/chat-folders.test.ts` (the pure model — the exactly-once partition **at every level**, the unknown-folder and unknown-*parent* fallbacks, the cycle and depth repairs, the move rules, subtree deletion, order preservation inside a partition, the cap and the defensive parsers) and `frontend/tests/e2e/sidebar-chat-folders.spec.ts` (create from the `+` — including where it sits relative to the heading, measured, and on a workspace with no conversations at all — drag a chat in and confirm it left the root list, collapse/expand by the arrow, drag back out, rename and delete from the folder menu, the row menu's non-drag path, reload persistence, and the nesting workflow: a subfolder rendering inside its parent's own child area, a folder dragged in and back out again, and a parent's delete returning the chats from its subfolders).

The two halves the pure model cannot see are pinned separately:

- `backend/tests/test_user_ui_state.py` — the registry in the shared per-user bag: cold-cache round-trip in display order, per-user isolation, an explicit empty list persisting, malformed/duplicate/oversized input degrading rather than raising, folders and tabs not clobbering each other in the one file, and the tree repairs (a nested folder round-tripping with **no** `parentId` key on a top-level one, an unknown or wrong-typed parent promoting rather than dropping, a cycle broken, an over-deep chain pulled back from the top of the breach).
- `backend/tests/test_chat_folders_settings_router.py` — the routes: round-trip (including `parentId`, which a route that drops it flattens every subfolder on the next reload), the normalized response being authoritative — the repair has to be visible in the body the client adopts, not only in the stored file — the cap, an explicit clear, per-user scoping, and an email-shaped identity not breaking per-user path resolution.
...
Then end-to-end (`make dev`): click **New folder** beside the sidebar's "Recent chats" heading, name a folder, drag a conversation onto it → it disappears from the list and appears under the folder; click the arrow to collapse and expand; drag it back onto the list below → it leaves the folder; make a second folder and drag it onto the first → it nests inside and indents, and collapsing the parent hides it; file a chat into the subfolder and confirm the parent's count includes it; rename and delete the parent from its **⋯** menu and confirm the conversations come back rather than going with it; reload and confirm the tree and its contents are still there.
...
### 36. A conversation remembers which model it was on

Every chat already had its own model, subagent model, mode and reasoning effort (§16): selecting one in an open chat never touched another. But the selection lived only in `localStorage`, which knows about the conversations **this browser** has touched, under **this origin**, since the last time site data was cleared. Everywhere else there was no override to read, so the composer fell back to the app default — and opening an older conversation showed whichever model happened to be selected last rather than the one it had been talking to.

Two things were wrong, and the second one was worse because it happened on every single new chat:

- **A conversation this browser had not seen had no record at all.** Reaching the same server from a phone, from a second browser, or after a site-data clear meant every chat looked identical. So the selection is now written onto the **thread's own metadata** (`deerflow_workflow`) as well: model, subagent model, mode, reasoning effort. A chat with no local override adopts what the conversation recorded; a chat that *has* one keeps it, because that is what the user is looking at right now. The local store stays the fast path — it is read synchronously during render, while metadata arrives with a fetch — so this is a seed, not a replacement.
- **A new chat forgot its model on its first turn.** A brand-new chat is addressed by a client-generated placeholder id until the first send, and the backend then assigns a *different* id. Every per-conversation key is stored under the thread id, so the moment the conversation became real its selection was orphaned under the placeholder and the composer snapped back to the app default — having just answered on the model the user picked. `onStart` now carries the override across (`copyThreadContextOverride`) and records it under the real id.

**A fallback is not a choice.** The composer resolves a model and mode the moment the model list loads, before a chat's metadata has arrived — on a second device, on every chat after the first, because the model list is cached app-wide. Passwordless mode has no account sync, and there that fallback used to be pinned as the chat's own override, so the recorded workflow was refused as "this browser already has a selection" and the next edit PATCHed the fallback over the record. `resolveThreadContext` now repairs only keys the chat already overrides and puts everything else in the in-memory base.

**Changing a model is not activity.** The sidebar is recency-ordered, so a metadata PATCH that bumped `updated_at` would shove the conversation to the top the moment its model changed — reshuffling the list under the user's cursor. The workflow key therefore joins the pin/archive/folder keys in the Gateway's narrow "placement" exemption, with its own strict value guard: an unknown key or a non-string value falls through to the ordinary path rather than inheriting the exemption. That guard is the load-bearing part — a loose one hands the exemption to arbitrary metadata writes and the sidebar quietly stops re-sorting.

| Piece                          | Where                                                                                            |
| ------------------------------ | ------------------------------------------------------------------------------------------------ |
| Pure model (read/narrow/diff)  | `frontend/src/core/threads/thread-workflow.ts`                                                   |
| Seed + record + promotion      | `frontend/src/components/workspace/chats/use-thread-workflow-memory.ts`, wired in `chat-instance.tsx` |
| Metadata key + value guard     | `backend/packages/harness/deerflow/persistence/thread_meta/base.py` (`is_valid_thread_workflow`) |
| PATCH exemption                | `backend/app/gateway/routers/threads.py` (`_UI_PLACEMENT_METADATA_GUARDS`)                        |

**Verify it works.**

```bash
cd backend  && uv run pytest tests/test_thread_workflow_metadata.py -q
cd frontend && pnpm test thread-workflow thread-context-account-boundary
cd frontend && pnpm test:e2e thread-workflow-memory
```

Then end-to-end: pick a distinctive model in a chat and send; open a different conversation and confirm the picker changes to *that* chat's model rather than staying on the last one; start a new chat, pick a model, send, and confirm the picker still shows it after the answer lands.

### 37. Updating the browser and search image on demand

Camoufox and the bundled SearXNG image already refresh themselves — daily, on a timer or throttled at launch (see *Automatic updates*). This adds the **Settings → Maintenance → Update now** button for the case that throttle is in the way: page fetches started failing, or search stopped returning results, and the answer is "pull the newer build now" rather than "wait until tomorrow". Without it the only route was an SSH session on the host running `make auto-update`, which is precisely the thing a self-hosted box is supposed to spare you.

It drives `scripts/update_camoufox_searxng.py` — the same implementation the Makefile target and the launch hook use, imported by path rather than reimplemented, so the three cannot drift. Four properties hold it up, each a way the button could be worse than not existing:

- **Single-flight.** Two concurrent `camoufox fetch` / `docker compose pull` runs fight over the same files, and nothing downstream would report the corruption. A second request while one is in flight is refused, not queued.
- **The POST returns without doing the work.** A Docker pull takes minutes; holding the HTTP request open outlives every reverse-proxy read timeout, so the user would see a 504 for a refresh that actually succeeded. The request starts the run and the page polls — and only while a run is in flight, so an idle settings page never talks to the gateway on a timer.
- **One component cannot sink the other.** A machine with no Docker still gets its browser refreshed; a raising or hanging component is reported as `failed`/`timeout` and the other still runs. The outcome per component is shown, including "not in use — skipped", because a silent success on a component that was never touched is a lie.
- **Admin-gated, and no caller input.** This runs commands on the host. The work is a fixed pair of actions with no parameters, so nothing from the request reaches a command line, and the route is gated like multi-user mode on top of that.

| Piece                | Where                                                                              |
| -------------------- | ---------------------------------------------------------------------------------- |
| Single-flight runner | `backend/app/gateway/dependency_update.py`                                         |
| Routes               | `GET`/`POST /api/settings/update-dependencies` in `backend/app/gateway/routers/settings.py` |
| Client               | `frontend/src/core/settings/dependency-update.ts`                                  |
| Settings page        | `frontend/src/components/workspace/settings/maintenance-settings-page.tsx`         |

**Verify it works.**

```bash
cd backend && uv run pytest tests/test_dependency_update.py -q
```
...
### 39. A search backup for a blocked connection

§31 fixed the *defaults* that made search fail on a VPN — the engine mix, the
timeout, and reporting a blocked engine as an error rather than an empty
success. It did not fix the underlying fact, which is that **SearXNG scrapes
consumer engines and therefore inherits your IP's reputation**. From a
commercial VPN exit or any datacenter range, Google, DuckDuckGo, Brave and
Startpage can all block at once; no engine mix survives that, because the block
is at the network tier, before any query is parsed. §31's error is the correct
behaviour and it still leaves the agent with no results.

So `web_search` gains what `web_fetch` already had: a dispatcher with a second
backend. `deerflow.community.web_search.tools:web_search_tool` resolves
`backend:` and an optional `fallback:`, with `DEER_FLOW_WEB_SEARCH_BACKEND`
overriding both — the same resolution order, the same shape, deliberately, so
there is one pattern to learn rather than two.

**The shipped ordering is Tavily first, SearXNG behind it**, while the *code*
default stays `searxng` so a `config.yaml` predating the dispatcher keeps
working untouched. The ordering is a judgement, not a law: a keyed API is
authenticated and so is immune to the IP reputation problem above, which makes
it the one that still answers from a blocked exit. It costs privacy — with this
ordering a query reaches a third party first — and the swap back is two values,
documented in `config.example.yaml` and the README beside the trade itself.

**An unconfigured primary promotes the fallback rather than failing.**
`_backend_is_available` gates *both* slots. A Tavily primary with no
`TAVILY_API_KEY` is not attempted-and-failed; it is skipped, and SearXNG runs as
the only backend with no error at all. This is what makes a Tavily-first default
safe to ship: a fresh clone that has never seen a key searches locally and works,
instead of failing every query on a credential it was never given. Promotion also
consumes the fallback slot, so the promoted backend runs exactly once and reports
for itself — `test_the_promoted_fallback_is_not_retried_as_its_own_fallback`.
The one case that is still attempted is a keyless primary with *no* fallback
configured: there is nothing to promote, and skipping it would turn a missing key
into silence instead of the error the user needs.

**The fallback fires on a raised primary, never on an empty result.** This is
the whole design, and it is §31's distinction re-used one layer up. SearXNG
answers `HTTP 200` with an empty array both when nothing matched and when every
engine was blocked; §31 made the second case raise
`SearxngEnginesUnavailableError` while leaving the first an ordinary success.
The dispatcher keys off exactly that: an exception is a real failure worth
falling back from, an empty list is an answer. Collapsing the two — "no results,
try the backup" — is the tempting simplification and it is silently expensive:
every genuinely zero-match query would spend a metered third-party credit and,
worse, **leave the machine**, which is the one thing the local-first default
exists to prevent. `test_an_empty_result_set_does_not_trigger_the_fallback`
is the load-bearing test, and it is the only thing that goes red when someone
makes that change.

**A key-bearing backend is inert without its key, in either slot.**
`_backend_is_available` asks the provider, and Tavily answers from tool config
or `TAVILY_API_KEY`. In the *fallback* slot that means a stack naming
`fallback: tavily` without a key behaves as if the fallback were absent: it is
skipped and **the primary's own failure is reported verbatim**, because a Tavily
credentials error would bury the failure the user actually needs to read. That
resolution runs *outside* the dispatcher's try block, so `_web_search_extras`
swallows a config error and lets the env var answer — a missing `config.yaml`
must not take down `web_search` through an availability check.

**Tavily's SDK is synchronous**, so `search_via_tavily` wraps it in
`asyncio.to_thread`. A blocking HTTP call on the agent's event loop stalls every
other in-flight run; `make test-blocking-io` exists for this class of bug, and
`test_search_via_tavily_is_a_coroutine_function` fails loudly if a refactor
unwraps it rather than letting it degrade into a production stall.

**The silent break this change nearly shipped.** `scripts/detect_searxng.py`
decides whether to auto-start the bundled container by grepping the active
config for `deerflow.community.searxng`. The dispatcher names
`deerflow.community.web_search` and selects the engine with a `backend:` key, so
the shipped `config.example.yaml` read as "no SearXNG" and **every launch path
stopped starting the container** — with nothing red anywhere, because `skip` is
a legitimate outcome of that script. `config_uses_searxng` now understands both
shapes, including an omitted `backend:` (searxng is the default) and SearXNG as
the *fallback* of a Tavily-primary stack. The match is scoped to the
`- name: web_search` block rather than the whole file, because `backend:` is not
unique to this entry — `web_fetch` carries `backend: camoufox` — and a file-wide
scan would read a neighbour's key as this one's and start a container nothing
uses. `scripts/wizard/providers.py` writes the dispatcher entry too, so
`make setup` and `make config` produce the same search behaviour.

`scripts/doctor.py` had the same shape of gap, one notch less severe:
`check_web_tool` matches a provider by substring of `use`, and the dispatcher's
path contains neither `searxng` nor `tavily`, so the check still returned `ok`
but with an **empty description** — a diagnostic tool reporting nothing about
what is configured. It now carries a `community.web_search.tools` entry in
`free_providers`, exactly as the web_fetch dispatcher already did. The pattern
worth remembering: **anything that identifies a provider by grepping the `use`
...
and keeps its own answer (`fallback: jina`, §31, plus the stealth options in
§40). Stock quotes are unfixed by either: they fail at the network tier on a
plain HTTP request, so they need a keyed data API rather than a better browser.

### 40. Camoufox was a stealth browser with its stealth switched off

`web_fetch`'s default backend is Camoufox, a Firefox fork whose entire purpose
is defeating bot detection. It was launched like this, and this was the whole
configuration:

```python
AsyncCamoufox(headless=True)
```

Every anti-detection option Camoufox offers was left at its default. The
expensive one is **`geoip`**: with it, Camoufox reads the exit IP and aligns
timezone, locale and geolocation to it. Without it the browser reports the
*host's* timezone and locale while the connection leaves from wherever the VPN
chose — a Swiss exit IP on a browser claiming a different timezone. That
mismatch is a stronger and cheaper signal than anything the fingerprint layer
can fix, and it was being handed over on every fetch by a tool installed
specifically to avoid it. `humanize` (cursor motion) and `block_webrtc` (the
classic way a real address leaks out from behind a proxy) were likewise unused.

All three now default on, and every one is overridable from a `camoufox:` block
on the `web_fetch` entry, with `null` handing a decision back to Camoufox.

**The retry is the part that matters.** These options can fail for reasons that
have nothing to do with the page being fetched: an unsupported keyword on an
older Camoufox, a geoip database that was never downloaded, a `proxy` that is
refusing connections. §31 is the cautionary tale — a browser that was *present*
but unrunnable made every fetch fail while every presence check passed. So a
launch that raises with stealth options is retried **bare**, which is exactly
how it launched before this section existed, and the failure is logged loudly
enough to act on. Stealth is optional; `web_fetch` is not.

Two things that retry must not do, both tested:

- **It must not mask a missing browser.** `_looks_like_missing_browser` is
  checked *before* the retry, so the actionable "run `make fetch-browser`"
  message survives instead of becoming a second identical failure
  (`test_a_missing_browser_still_raises_and_is_not_retried`).
- **It must not fire when there is nothing to degrade to.** With every option
  set to `null` the launch carries no stealth, so a failure is the answer rather
  than the trigger for a pointless second attempt.

**The `proxy` key is deliberately namespaced.** `web_fetch` already documents a
top-level `proxy:` — for the *jina* backend, a different process on a different
network path. Reading that key here would silently repoint a user's jina proxy
at the browser, so Camoufox's proxy lives inside the `camoufox:` block and
`test_the_jina_proxy_key_does_not_reach_the_browser` keeps it that way.

**Not verified end to end in CI.** The option *assembly* and the degradation
path are unit-tested against a stubbed `AsyncCamoufox`; a real launch with
`geoip=True` needs the browser binaries and a geoip database, which the test
environment does not have. The retry is what makes that acceptable: the
un-verified path fails safe, into the launch that was shipping before.
