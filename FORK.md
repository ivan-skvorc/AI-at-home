...
- **2026-08-20 — partial.** Offline half clean: `scripts/audit_models.py` reported no drift (display-name/price agreement and two-source parity both hold), the stale-fixture self-test still surfaces all four drift kinds, and `sync-api-key-models.py --dry-run` plus the four regression suites are green. **Anthropic block fully verified** against the provider's current model list — all six slugs (`claude-fable-5`, `claude-opus-5`, `claude-opus-4-8`, `claude-sonnet-5`, `claude-sonnet-4-6`, `claude-haiku-4-5`), all six price pairs, the 0.1x cache-read rates, and Sonnet 5's `$2/10` intro window through **2026-08-31** all match; the roster shape (Opus/Sonnet keep last-4.x + current-5, Haiku/Fable latest only) is correct as-is, and Mythos 5 stays out because it is invitation-only, so a normal `ANTHROPIC_API_KEY` cannot reach it. **Every other provider unverified:** the environment this pass ran in blocks egress to `openrouter.ai` and to all eleven first-party provider hosts, so no figure could be read off a provider's own page. Per the "verify, never invent" rule, nothing was changed for them — a price that is wrong *with confidence* silences the next audit, which is worse than one that is merely stale.

  **Carried forward for the next pass on an unrestricted host** — candidate roster changes that surfaced but could not be confirmed, so they are leads, not findings: **Grok 4.6** (xAI, reported 2026-08-12) against the bundled Grok 4.5; **Qwen3.8-Max** (2026-08-03) against Qwen3.7 Max; **GLM-5.3** (z.ai, 2026-08-14) against GLM-5.2; and **Mistral Medium 3.5** against Mistral Medium 3. Each would be a flagship roll-forward under the *Which models to keep in the bundle* rule, and each needs its exact slug **and** price read off the provider's page before it lands in either source.
