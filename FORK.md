...
| **A chat remembers its own model** (§36)                                                                 | `cd backend && uv run pytest tests/test_thread_workflow_metadata.py` + `cd frontend && pnpm test thread-workflow`. The selection is recorded on the thread (`deerflow_workflow`) as well as in `localStorage`, so a conversation opened on another browser/device shows **its** model rather than the app default. Two silent failures: the value guard must stay strict (it decides the `updated_at` exemption — a loose one hands that exemption to arbitrary metadata and the sidebar stops re-sorting on real activity), and the new→real promotion must carry the selection across (`copyThreadContextOverride` + `recordOnPromotion` in `chat-instance.tsx`'s `onStart`) or a new chat forgets its model on its first turn. `workflowChanged` is what stops the composer PATCHing on every render — neutralize it and `thread-workflow-memory.dom.test.tsx` goes red. |
...
cd frontend && pnpm test thread-workflow
```
