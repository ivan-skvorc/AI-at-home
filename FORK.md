...
Pinned by `frontend/tests/unit/core/models/sorting.test.ts` (price/provider parsing incl. the promo pair and bare-version-number guard, name/price/default sorting, unpriced-last, `demoteLast`, provider grouping, and `splitModelNamePriceSegments` — single vs. promo pair, the exact-reassembly property, no-price and empty names, and no shared regex state between calls).
