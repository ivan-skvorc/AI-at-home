...
| **SearXNG default `web_search`**                                                                        | active `web_search` runs through the dispatcher (`deerflow.community.web_search.tools:web_search_tool`), so two things must hold or search dies in silence: `cd backend && uv run pytest tests/test_web_search_dispatcher.py tests/test_detect_searxng.py -q`. (1) `scripts/detect_searxng.py` must recognize the dispatcher shape — it picks the engine with `backend:`/`fallback:` rather than naming the provider module, so a matcher that only greps `deerflow.community.searxng` reads the shipped config as "no SearXNG" and stops auto-starting the container on every launch path, with nothing red because `skip` is a legitimate outcome (`TestDispatcherConfigIsRecognized`, and `test_a_neighbouring_tools_backend_key_does_not_leak_in` because web_fetch carries a `backend:` too). (2) the Tavily fallback fires **only** on a raised primary, never on an empty-but-successful search (`test_an_empty_result_set_does_not_trigger_the_fallback`), and stays inert without `TAVILY_API_KEY` (`test_a_keyless_tavily_fallback_is_skipped_and_the_primary_error_stands`); collapsing either sends every zero-match query to a metered third party and off the machine.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`backend:` (default `searxng`) and an optional `fallback:`, with
`DEER_FLOW_WEB_SEARCH_BACKEND` overriding both — the same resolution order, the
same shape, deliberately, so there is one pattern to learn rather than two.
...
**A key-bearing fallback is inert without its key.** `_fallback_is_available`
asks the provider, and Tavily answers from tool config or `TAVILY_API_KEY`. A
stack that names `fallback: tavily` without ever setting the key behaves exactly
as it did before this section existed: the fallback is skipped and **SearXNG's
own failure is reported verbatim**. Reporting a Tavily credentials error instead
would bury the failure the user actually needs to read. That resolution runs
*outside* the dispatcher's try block, so `_web_search_extras` swallows a config
error and lets the env var answer — a missing `config.yaml` must not take down
`web_search` through the backup's availability check.
...
`locale` options are unused, so the browser advertises host locale and timezone
while exiting in whatever country the VPN chose. That mismatch is itself a
detection signal and is worth a future cycle; it is out of scope here.
